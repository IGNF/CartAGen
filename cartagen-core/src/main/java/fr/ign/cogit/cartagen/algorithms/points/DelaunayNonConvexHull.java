...
public class DelaunayNonConvexHull {
  ...
  public IGeometry compute() {
    ...
    if (DelaunayNonConvexHull.logger.isTraceEnabled()) {
      DelaunayNonConvexHull.logger.trace(
          "Computation of the non-convex hull using Delaunay Triangulation and edge removal");
    }
    ...
    while (isAvailable.contains("true")) {
      ...
      while (markTriangles.hasNext()) {
        ...
        if (((IPolygon) hull).getExterior()
            .intersects(triangle.getPoint1().getGeom())
            && ((IPolygon) hull).getExterior()
                .intersects(triangle.getPoint2().getGeom())
            && ((IPolygon) hull).getExterior()
                .intersects(triangle.getPoint3().getGeom())) {
          continue;
        }
        ...
        // 2nd case
        else if (((IPolygon) hull).getExterior()
            .intersects(triangle.getPoint1().getGeom())
            && ((IPolygon) hull).getExterior()
                .intersects(triangle.getPoint3().getGeom())) {
          double length = triangle.getPoint1().getGeom()
