 ...
 * @copyright IGN
 ******************************************************************************/
package fr.ign.cogit.cartagen.mrdb.enrichment;
...
public class CorrectSliversProcess extends ScaleMasterPreProcess {
  ...
  public void execute(CartAGenDB dataset) throws Exception {
    ...
    for (Class<? extends IGeneObj> classObj : this.getProcessedClasses()) {
      IPopulation<IGeneObj> pop = dataset.getDataSet()
          .getCartagenPop(dataset.getDataSet().getPopNameFromClass(classObj));
      ...
      for (IGeneObj obj : pop) {
        ...
        if (doublePt != null) {
          ...
          CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool()
              .addFeatureToGeometryPool(pol, Color.RED, 4);
        }
      }
    }
  }
