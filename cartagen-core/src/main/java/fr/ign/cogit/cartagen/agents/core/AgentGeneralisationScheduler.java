 ...
 */
public class AgentGeneralisationScheduler extends Scheduler implements AgentObservationSubject {
    private static Logger logger = Logger.getLogger(AgentGeneralisationScheduler.class.getName());
    ...
    public synchronized void run() {
        ...
        if (AgentGeneralisationScheduler.logger.isDebugEnabled()) {
            AgentGeneralisationScheduler.logger.debug("  nombre d'agents a activer: " + nb);
        }
        if (AgentGeneralisationScheduler.logger.isDebugEnabled()) {
            AgentGeneralisationScheduler.logger.debug("  echelle cible: " + Legend.getSYMBOLISATI0N_SCALE());
        }
        ...
        try {
            ...
            while (!this.getList().isEmpty()) {
                ...
                if (AgentGeneralisationScheduler.logger.isInfoEnabled()) {
                    AgentGeneralisationScheduler.logger.info("Activation de " + agent + " (" + i++ + "/" + nb + ")");
                }
                agent.activate();
