package fr.ign.cogit.cartagen.algorithms.rail;
...
public class TypifySideTracks {
  ...
  private Set<TriageArea> identifyTriageAreas() {
    ...
    for (IRailwayLine rail : dataset.getRailwayLines()) {
      ...
      if (geom instanceof IPolygon)
        ...
        triageAreas.add(new TriageArea((IPolygon) geom.buffer(1.0), triageLines,
            rootRails, attributeNames));
    }
  ...
  private void typifyTriageArea(TriageArea area) {
    ...
    double minDist = symbolWidth
        + separationDist * Legend.getSYMBOLISATI0N_SCALE() / 1000;
    for (Stroke stroke : strokeList) {
      ...
      Collection<Stroke> neighbours = strokeFc
          .select(stroke.getGeom().buffer(minDist));
  ...
  private void enrichAndMakePlanar() {
    ...
    NetworkEnrichment.buildTopology(dataset, dataset.getRailwayNetwork(), false,
        factory);
