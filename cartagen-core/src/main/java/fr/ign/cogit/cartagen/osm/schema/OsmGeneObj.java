  ...
  public Object getAttribute(String nomAttribut) {
    ...
    if (((AttributeType) attribute).getNomField().length() == 0) {
      ...
      nomFieldMaj = Character
          .toUpperCase(((AttributeType) attribute).getNomField().charAt(0))
    ...
    while (!classe.equals(Object.class)) {
      try {
        ...
        // logger.debug("La m�thode "+nomGetFieldMethod+" n'existe pas dans la
        // classe "+classe);
    ...
    while (!classe.equals(Object.class)) {
      try {
        ...
        // if (logger.isDebugEnabled())
        // logger.debug("La méthode "+nomGetFieldMethod+" n'existe pas dans la
        // classe "+classe);
      } catch (IllegalAccessException e) {
        if (AbstractFeature.logger.isDebugEnabled()) {
          AbstractFeature.logger
