 ...
 * @copyright IGN
 ******************************************************************************/
package fr.ign.cogit.cartagen.appli.core.geoxygene.selection;
...
import fr.ign.cogit.cartagen.appli.utilities.OGCFilterPanel;
...
public class AttributeQueryFrame extends JFrame
    ...
    private String lblNone, lblWindow, lblGeometry, lblEnvelope, classTitle, lblClasses, lblMessage;

    public AttributeQueryFrame(GeOxygeneApplication appli) {
        super(I18N.getString("AttributeQueryFrame.menuName"));
        this.setSize(800, 600);
        this.internationalisation();
        this.setGeoClasses();
        this.dbName = CartAGenDoc.getInstance().getCurrentDataset().getCartAGenDB().getName();
        this.appli = appli;
        ...
        Border titled = BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED),
                this.classTitle);
        this.pClass.setBorder(BorderFactory.createCompoundBorder(titled, BorderFactory.createEmptyBorder(5, 5, 5, 5)));
        this.pClass.setLayout(new BoxLayout(this.pClass, BoxLayout.X_AXIS));

        // ***********************************
        // a panel to define the OGC Filter
        this.pFilter = new OGCFilterPanel(this, (Class<?>) this.classJList.getSelectedValue());

        // ***********************************
        // a panel to define the extent of the query
        this.pExtent = new JPanel();
        JPanel pDb = new JPanel();
        this.cbDbs = new JComboBox(CartAGenDoc.getInstance().getDatabases().keySet().toArray());
        this.cbDbs.setSelectedItem(CartAGenDoc.getInstance().getCurrentDataset().getCartAGenDB());
        ...
        this.pExtent.add(Box.createVerticalGlue());
        titled = BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED),
                this.extentTitle);
        this.pExtent.setBorder(BorderFactory.createCompoundBorder(titled, BorderFactory.createEmptyBorder(5, 5, 5, 5)));
    ...
    public void valueChanged(ListSelectionEvent e) {
        if (e.getSource().equals(this.classJList)) {
            if (!e.getValueIsAdjusting()) {
                this.pFilter.changeSelectedClass((Class<?>) this.classJList.getSelectedValue());
            }
        }
    }
    ...
    private void triggerQuery() {
        ...
        String popName = dataset.getPopNameFromClass(this.pFilter.getFilteredClass());
        ...
        if (this.extentType != ExtentType.NONE) {
            ...
            if (this.extentType == ExtentType.WINDOW) {
                geom = appli.getMainFrame().getSelectedProjectFrame().getLayerViewPanel().getViewport()
                        .getEnvelopeInModelCoordinates().getGeom();
            }
            if (this.extentType == ExtentType.ENVELOPE) {
                geom = new GM_Envelope(Double.valueOf(this.txtXMin.getText()), Double.valueOf(this.txtXMax.getText()),
                        Double.valueOf(this.txtYMin.getText()), Double.valueOf(this.txtYMax.getText())).getGeom();
            }
            if (this.extentType == ExtentType.GEOMETRY) {
                geom = appli.getMainFrame().getSelectedProjectFrame().getLayerViewPanel().getSelectedFeatures()
                        .iterator().next().getGeom();
            }
        ...
        for (IGeneObj obj : queryColn) {
            ...
            if (this.pFilter.getFilter() == null) {
                appli.getMainFrame().getSelectedProjectFrame().getLayerViewPanel().getSelectedFeatures().add(obj);
                nbSelectedObjs++;
                continue;
            }
    ...
    /**
     * Get all the classes inheriting from IGeneObj in the project and set them
     * as the geo classes to be queried.
     */
