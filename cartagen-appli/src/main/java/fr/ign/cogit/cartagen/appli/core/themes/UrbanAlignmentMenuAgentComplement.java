...
public class UrbanAlignmentMenuAgentComplement {
    ...
    private JMenuItem mGeneraliseBuildings = new JMenuItem(new GeneraliseBuildingsAction());

    public UrbanAlignmentMenuAgentComplement() {

        UrbanAlignmentMenu menu = DataThemesGUIComponent.getInstance().getUrbanAlignmentMenu();
                    ...
                    for (IFeature sel : SelectionUtil
                        ...
                        if (sel instanceof IUrbanAlignment) {
                            ...
                            for (IUrbanElement build : align.getUrbanElements()) {
                                GeographicAgentGeneralisation ab = AgentUtil.getAgentFromGeneObj(build);
                                if (ab != null) {
                                    try {
                                        ab.activate();
