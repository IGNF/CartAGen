...
public class LineCurvatureSmoothing {
  ...
  public void compute(boolean isPersistantObject) {
    ...
    if (!(geomStart instanceof ILineString)) {
      if (LineCurvatureSmoothing.logger.isInfoEnabled()) {
        LineCurvatureSmoothing.logger.info(
            "Impossible to perform Curvature smoothing - non linear geometry");
      }
      return;
    }
    if (geomStart.coord().size() < 3) {
      if (LineCurvatureSmoothing.logger.isInfoEnabled()) {
        LineCurvatureSmoothing.logger.info(
            "Impossible to perform Curvature smoothing - not enough points on geometry");
      }
      return;
    }
    ...
    // Algorithm
    ILineString geomResult = LineCurvatureSmoothing
        .lineCurvatureSmoothing(geoxLine, this.symbolWidth, this.resolution);
    ...
    if (geomResult != null) {
      ...
      if (LineCurvatureSmoothing.logger.isInfoEnabled()) {
        LineCurvatureSmoothing.logger
            .info("Curvature smoothing performed on " + this.geneObj);
      }
    }
  ...
  public static ILineString lineCurvatureSmoothing(ILineString ls,
    ...
    for (int k = 0; k < ls.coord().size() - 1; k++) {
      direction[k] = Math.atan2(
          ls.coord().get(k + 1).getY() - ls.coord().get(k).getY(),
          ls.coord().get(k + 1).getX() - ls.coord().get(k).getX());
    }
    ...
    coordsRes[0] = new Coordinate(ls.coord().get(0).getX(),
        ls.coord().get(0).getY());
    double angle = 0.0;
    ...
    LineString lsRes = new GeometryFactory()
        .createLineString(dp.getResultGeometry().getCoordinates());

    // rotation (en principe, tres faible)
    double angle1 = Math.atan2(
        ls.coord().get(ls.coord().size() - 1).getY() - ls.coord().get(0).getY(),
        ls.coord().get(ls.coord().size() - 1).getX()
            - ls.coord().get(0).getX());
    coordsRes = lsRes.getCoordinates();
    double angle2 = Math.atan2(
        coordsRes[coordsRes.length - 1].y - coordsRes[0].y,
        coordsRes[coordsRes.length - 1].x - coordsRes[0].x);
    angle = angle1 - angle2;
    if (angle < -Math.PI) {
      angle += 2 * Math.PI;
    }
    ...
    lsRes = CommonAlgorithms.rotation(lsRes,
