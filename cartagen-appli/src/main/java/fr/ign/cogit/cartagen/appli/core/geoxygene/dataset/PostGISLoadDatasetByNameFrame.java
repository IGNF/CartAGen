...
public class PostGISLoadDatasetByNameFrame extends JFrame implements ActionListener, ListSelectionListener {
    ...
    private JTextField txtDbName, txtHost, txtPort, txtPwd, txtUser, txtSLD, txtXML, txtCartagenDbName, txtSchema;
    private JList<String> dList, aList;
    private JButton addDataset, removeDataset, connection, btnOk, btnCancel, btnSLD, btnXML;
    private JPanel pConnection, pConnectButton, pDatasets, pButtons, pPanels, pScale, pMapping, pNewWin;
    ...
    // CONSTRUCTOR
    // *****************************************
    public PostGISLoadDatasetByNameFrame() throws HeadlessException, NoSuchMethodException, SecurityException {
        ...
        btnXML = new JButton(new ImageIcon(this.getClass().getClassLoader().getResource("images/browse.jpeg")));
        ...
        btnSLD = new JButton(new ImageIcon(this.getClass().getClassLoader().getResource("images/browse.jpeg")));
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().equals("cancel")) {
            ...
            proposList.clear();
            loader = new PostGISLoader(txtHost.getText(), txtPort.getText(), txtDbName.getText(), txtUser.getText(),
                    txtPwd.getText(), txtSchema.getText());
            ...
            if (xmlFile == null) {
                txtXML.setText("xml/mapping_test_data.xml");
                xmlFile = new File(this.getClass().getClassLoader().getResource(txtXML.getText()).getPath());
            }

            // Creates the mapping by reading the xml file
            MappingXMLParser calacMappingXMLParser = new MappingXMLParser(xmlFile);
            ...
            Legend.setSYMBOLISATI0N_SCALE(((Integer) spinScale.getValue()).doubleValue());
            ...
            if (newWindow.getSelectedIndex() == 1) {
                ...
                PostGISCartAGenDB database = (PostGISCartAGenDB) doc.getDataset(dbName).getCartAGenDB();
                doc.setCurrentDataset(database.getDataSet());
                database.setLoader(loader);
                dataset = database.getDataSet();
                GeneObjImplementation implementation = mapping.getGeneObjImplementation();
            ...
            // in a new window
                ...
                PostGISCartAGenDB database = new PostGISCartAGenDB(txtCartagenDbName.getText());
    ...
    private void applySldToLoadedData(PostGISCartAGenDB database, File sldFile)
        ...
        List<IPopulation<? extends IFeature>> pops = database.getDataSet().getPopulations();
        for (int i = 0; i < pops.size(); i++) {
            ...
            NamedLayer layer;
            ProjectFrame pFrame = CartAGenPlugin.getInstance().getProjectFrameFromDbName(database.getName());
            if (pFrame.getLayer(pop.getNom()) != null) {
                ...
                if (database.getDataSet().getSld().getLayer(pop.getNom()) == null) {
                    ...
                    fts.getRules()
                            .add(LayerFactory.createRule(database.getDataSet().getGeometryTypeFromName(pop.getNom()),
                    ...
                    layer = (NamedLayer) database.getDataSet().getSld().getLayer(pop.getNom());
        ...
        if (sldFile != null) {
            ...
            StyledLayerDescriptor newSld = StyledLayerDescriptor.unmarshall(sldFile.getPath());
