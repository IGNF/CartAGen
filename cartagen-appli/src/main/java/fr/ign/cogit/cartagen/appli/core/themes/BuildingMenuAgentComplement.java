...
public class BuildingMenuAgentComplement {
    ...
    private Logger logger = Logger.getLogger(BuildingMenuAgentComplement.class.getName());

    private JLabel lblAgent = new JLabel("          AGENT");

    private JMenuItem mBatimentChargerTous = new JMenuItem(new LoadAction());
    private JMenuItem mBatimentRetourEtatInitialTous = new JMenuItem(new RestoreAction());

    public JCheckBoxMenuItem mPointsBatiVoir = new JCheckBoxMenuItem("Display points");
    public JCheckBoxMenuItem mSegmentsBatiVoir = new JCheckBoxMenuItem("Display segments");

    public JCheckBoxMenuItem mVoirSatisfactionBati = new JCheckBoxMenuItem("Display overall satisfaction");
    public JCheckBoxMenuItem mVoirCouleurSatisfactionBati = new JCheckBoxMenuItem("Display satisfaction color");

    public JCheckBoxMenuItem mVoirSatisfactionTaille = new JCheckBoxMenuItem("Voir satisfaction taille");

    public JCheckBoxMenuItem mVoirSatisfactionEquarrite = new JCheckBoxMenuItem("Voir satisfaction equarrite");

    public JCheckBoxMenuItem mVoirSatisfactionAltitude = new JCheckBoxMenuItem("Voir satisfaction altitude");

    public JCheckBoxMenuItem mVoirSatisfactionOrientation = new JCheckBoxMenuItem(
            "Voir satisfaction orientation generale");

    public JCheckBoxMenuItem mVoirSatisfactionElongation = new JCheckBoxMenuItem("Voir satisfaction elongation");

    public JCheckBoxMenuItem mVoirSatisfactionConvexite = new JCheckBoxMenuItem("Voir satisfaction convexite");

    public JCheckBoxMenuItem mVoirSatisfactionGranularite = new JCheckBoxMenuItem("Voir satisfaction granularite");
    ...
    private class LoadAction extends AbstractAction {
        ...
        public LoadAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Load all buildings in the agent scheduler");
    ...
    private class RestoreAction extends AbstractAction {
        ...
        public RestoreAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Restore all buildings to their previous state");
    ...
    private class SquaringAction extends AbstractAction {
            ...
            Thread th = new Thread(new Runnable() {
                ...
                public void run() {
                    String s = JOptionPane.showInputDialog(
                            ...
                            "Valeur tolérance angle (en degres)", "Cartagen", JOptionPane.PLAIN_MESSAGE);
                    double tol = 0.0;
                    if (s != null && !s.isEmpty()) {
                    ...
                    for (IFeature sel : SelectionUtil
                        ...
                        BuildingMenuAgentComplement.this.logger.info("Equarrissage du batiment " + sel);
                        try {
        ...
        public SquaringAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Trigger Squaring algorithm on selected buildings");
