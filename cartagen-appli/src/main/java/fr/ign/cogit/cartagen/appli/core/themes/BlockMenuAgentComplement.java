...
public class BlockMenuAgentComplement {
    ...
    private Logger logger = Logger.getLogger(BlockMenuAgentComplement.class.getName());

    private JLabel lblAgent = new JLabel("          AGENT");

    private JMenuItem mIlotChargerTous = new JMenuItem(new LoadAction());
    private JMenuItem mIlotRetourEtatInitialTous = new JMenuItem(new RestoreAction());

    public JCheckBoxMenuItem mIdIlotVoir = new JCheckBoxMenuItem("Display id");
    public JCheckBoxMenuItem mComponentsDisplay = new JCheckBoxMenuItem("Display buildings link");

    public JCheckBoxMenuItem mPointsIlotVoir = new JCheckBoxMenuItem("Display points");
    public JCheckBoxMenuItem mSegmentsIlotVoir = new JCheckBoxMenuItem("Display segments");
    public JCheckBoxMenuItem mSegmentsPlusCourtsIlotVoir = new JCheckBoxMenuItem("Voir segments plus courts");
    public JCheckBoxMenuItem mValeurSegmentsIlotVoir = new JCheckBoxMenuItem("Voir valeur segments");

    public JCheckBoxMenuItem mVoirTexteSatisfactionIlot = new JCheckBoxMenuItem("Voir satisfaction");
    public JCheckBoxMenuItem mVoirCouleurSatisfactionIlot = new JCheckBoxMenuItem("Voir couleur satisfaction");
    public JCheckBoxMenuItem mVoirSatisfactionComposants = new JCheckBoxMenuItem("Voir satisfaction composants");
    ...
    private JMenuItem mGeneralisationBatiments = new JMenuItem(new GeneralisationAction());
    private JMenuItem mDeplacementBatimentsGAEL = new JMenuItem(new DisplaceBuildingsGAELAction());
    ...
    private class LoadAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (IUrbanBlock obj : CartAGenDoc.getInstance().getCurrentDataset().getBlocks()) {
                AgentGeneralisationScheduler.getInstance().add(AgentUtil.getAgentFromGeneObj(obj));
            }
        }

        public LoadAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Load all blocks in the agent scheduler");
    ...
    private class RestoreAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            Thread th = new Thread(new Runnable() {
                ...
                public void run() {
                    for (IUrbanBlock obj : CartAGenDoc.getInstance().getCurrentDataset().getBlocks()) {
                        ((GeographicObjectAgentGeneralisation) AgentUtil.getAgentFromGeneObj(obj))
        ...
        public RestoreAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Restore all blocks to their previous state");
    ...
    private class GeneralisationAction extends AbstractAction {
            ...
            Thread th = new Thread(new Runnable() {
                ...
                public void run() {
                    for (IFeature sel : SelectionUtil
                        ...
                        BlockMenuAgentComplement.this.logger.info("Generalisation des batiments de l'ilot " + sel);
                        try {
        ...
        public GeneralisationAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Trigger buildings generalisation of the selected block agent");
            ...
            Thread th = new Thread(new Runnable() {
                ...
                public void run() {
                    for (IFeature sel : SelectionUtil
                        ...
                        BlockAgent ab = (BlockAgent) AgentUtil.getAgentFromGeneObj((IUrbanBlock) sel);
                        BlockMenuAgentComplement.this.logger.info("Déplacement des bâtiments de l'îlot (GAEL)" + sel);
                        try {
