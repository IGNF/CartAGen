...
public class LoadingFrame extends JFrame implements ActionListener {
    ...
    public void initFrame() {
        ...
        this.leftListPanel.setBorder(BorderFactory.createTitledBorder("Original layers"));
        ...
        this.selectionList.setSelectionMode(ListSelectionModel.SINGLE_INTERVAL_SELECTION);
        ...
        this.rightListPanel.setBorder(BorderFactory.createTitledBorder("Corresponding layers"));
        ...
        for (int i = 0; i < taille; i++) {
            ...
            this.list[i].setSelectionMode(ListSelectionModel.SINGLE_INTERVAL_SELECTION);
        ...
        for (int i = 0; i < taille; i++) {
            this.jtp.addTab(LoaderUtil.layerTypes().get(LoaderUtil.type[i]), this.list[i]);
        ...
        this.rightListPanel.add(
                new JLabel("<html><h2><i color='#084B8A'>Dataset being processed : " + new File(this.dataSet).getName()
                        + "</i></h2>" + "<p><i color='#009900'>Green : automated matching ensured</i><br/>"
    ...
    public void actionPerformed(ActionEvent ev) {
        try {
            ...
            // Modif Cecile: data should be loaded in src/resources loaded data,
            // not
            // in target, because they must be retrievable during a further
            // execution, even after a build (that would have deleted the
            // content of
            ...
            // dataset
            // New code:
            String destinationPath = "src/main/resources/loaded_data" + "/" + source.getName();
            ...
            if (ok) {
                // Traductions
                int taille = LoaderUtil.type.length;
                FileOutputStream fos = new FileOutputStream(destination + ".properties");
                for (int i = 0; i < taille; i++) {
                    LoaderUtil.traductionDe(fos, this.modele[i], destination, "\\" + LoaderUtil.type[i]);
                }
                fos.close();
                // Added Cecile: Now we can set the systemPath of the CartAGenDB
                // that
                ...
                // End added Cecile
                int rep = JOptionPane.showConfirmDialog(this, "Do you want to open this dataset ?");
                if (rep == JOptionPane.OK_OPTION) {
                    // Modif Cecile: LoadingFrame.cheminAbsolu and
                    // CartagenApplication.getInstance().cheminDonnees should
                    // not be
                    // modified here
                    // Old code:
     ...
     * 
     * @return true if the user wants to launch a chosen file that already
     *         exists false otherwise, meaning that traduction will be needed
     */
    ...
    private boolean chooseFile() {
        ...
        // Already traducted dataset
        if (this.dataSet.contains("CartAGen") && this.dataSet.contains("loaded_data")) {
            LoadingFrame.logger.info("The dataset exists and will be launched");
            EnrichFrame.getInstance().setVisible(true);
            return true;
        }
    ...
    private boolean test(String dataSetTest) {
        ...
        // Modif Cecile
        // LoadingFrame.cheminAbsolu should not be modified here + the path
        // search
        // for is corrupted
        // Old code:
        // Traducted file path
        ...
        String pathToTest = "src/main/resources/loaded_data" + File.separatorChar + datasetDirSimpleName;
