...
public class EmptySpacesDetection {
  ...
  public static void detectEmptySpaces(IUrbanBlock block,
    ...
    MorphologyTransform morph = new MorphologyTransform(bufferThreshold / 3,
        10);
    IGeometry blockGeom = block.getGeom();
    for (IUrbanElement element : block.getUrbanElements()) {
      blockGeom = blockGeom
          .difference(element.getGeom().buffer(bufferThreshold));
    }
    ...
    if (!(blockGeom instanceof IMultiSurface<?>)) {
      EmptySpacesDetection.logger.warn(
          "Problem, the geometry is neither a Polygon nor a Multisurface!");
      return;
    }
