...
import fr.ign.cogit.geoxygene.appli.plugin.cartagen.themes.HydroNetworkMenu;

public class HydroNetworkMenuAgentComplement {

  private Logger logger = Logger
      .getLogger(HydroNetworkMenuAgentComplement.class.getName());

  private JLabel lblAgent = new JLabel("          AGENT");

  private JMenuItem mResHydroCharger = new JMenuItem(new LoadHydroNetAction());
  private JMenuItem mResHydroChargerTroncons = new JMenuItem(
      new LoadHydroSectionsAction());
  private JMenuItem mVoirResHydroInfos = new JMenuItem(new DisplayInfoAction());
  private JMenuItem mResHydroRetourEtatInitial = new JMenuItem(
      new RestoreAction());
  public JCheckBoxMenuItem mSegmentsResHydroVoir = new JCheckBoxMenuItem(
      "Display segments");
  public JCheckBoxMenuItem mPointsResHydroVoir = new JCheckBoxMenuItem(
      "Display points");
  ...
  public JCheckBoxMenuItem mVoirResHydroEcoulement = new JCheckBoxMenuItem(
      "Voir reseau hydro indicateur ecoulement");
  ...
  public JCheckBoxMenuItem mVoirReliefEcoulement = new JCheckBoxMenuItem(
      "Voir relief indicateur ecoulement");
  ...
  private JMenuItem mResHydroDecomposition = new JMenuItem(
      new DecomposeAction());
  private JMenuItem mResHydroNettoyageDecomposition = new JMenuItem(
      new CleanDecompositionAction());
  private JMenuItem mTronconsHydroDeformation = new JMenuItem(
      new DeformSectionsAction());

  public HydroNetworkMenuAgentComplement() {

    HydroNetworkMenu menu = DataThemesGUIComponent.getInstance()
        .getHydroNetMenu();
  ...
  private class LoadHydroNetAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      HydroNetworkMenuAgentComplement.this.logger
  ...
  private class LoadHydroSectionsAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      AgentGeneralisationScheduler.getInstance().initList();
      for (IWaterLine at : CartAGenDoc.getInstance().getCurrentDataset()
          .getWaterLines()) {
    ...
    public LoadHydroSectionsAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Load the sections of a hydro network agent");
  ...
  private class DisplayInfoAction extends AbstractAction {
    ...
    public DisplayInfoAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Display information on the hydro network agent");
  ...
  private class RestoreAction extends AbstractAction {
    ...
    public RestoreAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Restore the hydro network agent to the previous state");
  ...
  private class DecomposeAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      Thread th = new Thread(new Runnable() {
        ...
        public void run() {
          HydroNetworkMenuAgentComplement.this.logger
              .info("Decomposition de reseau hydro");
          ((HydroNetworkAgent) AgentUtil.getAgentFromGeneObj(
              CartAGenDoc.getInstance().getCurrentDataset().getHydroNetwork()))
                  .decompose();
          HydroNetworkMenuAgentComplement.this.logger
              .info("Fin decomposition de reseau hydro");
        }
      });
      th.start();
    }
    ...
    public DecomposeAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Decompose the hydro network agent into sub-micro objects");
  ...
  private class CleanDecompositionAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      Thread th = new Thread(new Runnable() {
        ...
        public void run() {
          HydroNetworkMenuAgentComplement.this.logger
              .info("Nettoyage decomposition de reseau hydro");
          ((HydroNetworkAgent) AgentUtil.getAgentFromGeneObj(
              CartAGenDoc.getInstance().getCurrentDataset().getHydroNetwork()))
                  .cleanDecomposition();
          HydroNetworkMenuAgentComplement.this.logger
              .info("Fin nettoyage decomposition de reseau hydro");
        }
      });
      th.start();
    }
  ...
  private class DeformSectionsAction extends AbstractAction {
      ...
      Thread th = new Thread(new Runnable() {
        ...
        public void run() {
          ...
          for (IFeature sel : SelectionUtil.getSelectedObjects(
            ...
            IHydroSectionAgent tr = (IHydroSectionAgent) AgentUtil
                .getAgentFromGeneObj((IWaterLine) sel);
            HydroNetworkMenuAgentComplement.this.logger
                .info("Deformation de " + tr);
            if (HydroNetworkMenuAgentComplement.this.logger
    ...
    public DeformSectionsAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Deform the hydro sections with the GAEL model");
