package fr.ign.cogit.cartagen.osm.lodharmonisation.process;
...
public class AdjustLakeOutlineToPaths {
    ...
    for (LoDSpatialRelation instance : inconsistencies) {
      ...
      for (int i = 0; i < path_segments.size(); i++) {
        ...
        if (path_segments.get(i).intersects(lake.getGeom())) {
          ...
          if (i == 0) {
            while (i + 1 < path_segments.size() && GeometryFactory
                .buildCircle(path_segments.get(i + 1).getStartPoint(),
                    cushionRadius, 12)
                .intersects(lake.getGeom())) {
              diffGeom = controlledDifference(diffGeom, geomToSubstract(
                  path_segments.get(i + 1), lake.getGeom(), lakeSide));
              i++;
            }
          } else {
            int j = i;
            while (j > 0 && GeometryFactory
                .buildCircle(path_segments.get(j - 1).getEndPoint(),
                    cushionRadius, 12)
                .intersects(lake.getGeom())) {
              if (bridges.contains(path_segments.get(j - 1)))
                break;
              diffGeom = controlledDifference(diffGeom, geomToSubstract(
                  path_segments.get(j - 1), lake.getGeom(), lakeSide));
              j--;
      ...
      if (diffGeom instanceof IPolygon) {
        ...
        IPolygon simplePol = CommonAlgorithmsFromCartAGen
            .getBiggerFromMultiSurface(
                (IMultiSurface<IOrientableSurface>) diffGeom);
  ...
  public Set<Segment> findBridges(List<Segment> segList, IPolygon lake) {
    ...
    for (Segment segment : segList) {
      ...
      if (lake.contains(segment)) {
        ...
        IDirectPosition nearest = CommonAlgorithms.getNearestPoint(
            lake.getExterior().getPrimitive(), middlePt.toGM_Point());
        double distance = middlePt.distance2D(nearest);
        IDirectPosition nearestStart = CommonAlgorithms.getNearestPoint(
            lake.getExterior().getPrimitive(),
            segment.startPoint().toGM_Point());
        double distStart = middlePt.distance2D(nearestStart);
        IDirectPosition nearestEnd = CommonAlgorithms.getNearestPoint(
            lake.getExterior().getPrimitive(), segment.endPoint().toGM_Point());
        ...
        double orientStart = convertAngle(
            new Segment(nearestSegStart.startPoint(),
                nearestSegStart.endPoint()).orientation());
        ILineSegment nearestSegEnd = CommonAlgorithmsFromCartAGen
            .getNearestSegmentFromPoint(lake, segment.endPoint());
        double orientEnd = convertAngle(
            new Segment(nearestSegEnd.startPoint(), nearestSegEnd.endPoint())
                .orientation());
    ...
    for (int j = 0; j < segList.size(); j++) {
      ...
      ILineSegment nearestSegStart = CommonAlgorithmsFromCartAGen
          .getNearestSegmentFromPoint(lake, segment.startPoint());
      double orientStart = convertAngle(
          new Segment(nearestSegStart.startPoint(), nearestSegStart.endPoint())
              .orientation());
      ILineSegment nearestSegEnd = CommonAlgorithmsFromCartAGen
          .getNearestSegmentFromPoint(lake, segment.endPoint());
      double orientEnd = convertAngle(
      ...
      logger.fine("pass the orientation test in the second loop: " + orientation
          + " / " + orientStart + " & " + orientEnd);
      if (j != 0)
  ...
  private IPolygon geomToSubstract(Segment segment, IPolygon lake,
    ...
    if (lake.intersects(segment)) {
      ...
      IPolygon shorePolygon = BufferComputing
          .buildHalfOffsetBuffer(lakeSide.inverse(), segment, shoreBuffer);
  ...
  private Side computeLakeSide(Segment segment, IPolygon lake) {
    ...
    if (startPt == null) {
      ...
      startOutside = true;
      startPt = CommonAlgorithmsFromCartAGen
          .getCommonVertexBetween2Lines(segment, extLine);
    }
    // extend endPt first
    ...
    if (endPt == null) {
      ...
      // then, the intersection is taken instead
      endOutside = true;
      endPt = CommonAlgorithmsFromCartAGen.getCommonVertexBetween2Lines(segment,
          extLine);
    }
