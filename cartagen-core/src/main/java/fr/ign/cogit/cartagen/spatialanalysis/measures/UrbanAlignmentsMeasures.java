...
public class UrbanAlignmentsMeasures {
   ...
   *         d'un alignement
   */
  public static IGeometry getOverlappingGeometryInAlignment(IUrbanElement build,
    ...
    for (IUrbanElement ag : align.getUrbanElements()) {
      ...
      IGeometry intersection = CommonAlgorithms
          .buffer(ag.getFeature().getGeom(), distSep)
          .intersection(build.getGeom());
  ...
  public static double getEnlargedBuildingsOverlappingRateMean(
    ...
    for (IUrbanElement ag : align.getUrbanElements()) {
      ...
      initialGeoms.put(ag,
          new GM_Polygon(new GM_LineString(ag.getGeom().coord())));
   ...
   */
  public static double getBuildingsCentroidsDistanceSigma(
      IUrbanAlignment align) {
   ...
   */
  public static double getBuildingsMeanOrientationFactor(
      IUrbanAlignment align) {
    ...
    for (int i = 1; i < nonDeletedComponents.size(); i++) {
      // current orientation
      double orientation = new OrientationMeasure(
          nonDeletedComponents.get(i).getGeom()).getGeneralOrientation();
      // ideal orientation
      Angle idealAngle = new Angle();
      if (i == 0) {
        idealAngle = new Angle(nonDeletedComponents.get(0).getGeom().centroid(),
            nonDeletedComponents.get(0).getGeom().centroid());
      } else if (i == nonDeletedComponents.size() - 1) {
        idealAngle = new Angle(
            nonDeletedComponents.get(i - 1).getGeom().centroid(),
            nonDeletedComponents.get(i).getGeom().centroid());
      } else {
        idealAngle = new Angle(
            nonDeletedComponents.get(i - 1).getGeom().centroid(),
            nonDeletedComponents.get(i + 1).getGeom().centroid());
  ...
  public static IGeometry getRoadOverlappingGeometry(IUrbanAlignment align,
    ...
    for (INetworkSection tr : block.getSurroundingNetwork()) {
      ...
      IGeometry intersection = emp.buffer(distSep)
          .intersection(align.getSymbolGeom());
      if (geomSuperposition == null) {
        geomSuperposition = intersection;
      } else {
        geomSuperposition = intersection.union(geomSuperposition);
  ...
  public static double getRoadOverlappingRate(IUrbanAlignment align,
      IUrbanBlock block) {
    IGeometry overGeom = UrbanAlignmentsMeasures
        .getRoadOverlappingGeometry(align, block);
