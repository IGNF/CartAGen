...
public class VerticalHatchTexture extends BufferedImage {
   ...
   * @param thickness
   */
  public VerticalHatchTexture(Polygon pol, int offset, int bias, Color color,
      int thickness) {
    ...
    for (int i = 0; i < pol.getBounds2D().getWidth(); i += offset)
      for (int j = 0; j < thickness; j++)
        g.drawLine(i + j, 0, i + j + bias, (int) pol.getBounds2D().getHeight());
  }
