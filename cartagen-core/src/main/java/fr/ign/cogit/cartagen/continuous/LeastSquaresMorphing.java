...
public class LeastSquaresMorphing extends NonLinearLeastSquares
  ...
  public LeastSquaresMorphing(ILineString geomIni, ILineString geomFinal) {
    ...
    IDirectPosition anchor1 = new DirectPosition(
        this.geomIni.startPoint().getX(), this.geomIni.startPoint().getY());
    IDirectPosition anchor2 = new DirectPosition(this.geomIni.endPoint().getX(),
        this.geomIni.endPoint().getY());
  ...
  public IGeometry continuousGeneralisation(double t) {
    ...
    for (int i = 0; i < k; i++) {
      ...
      if (logger.isTraceEnabled()) {
        ...
        logger.trace(
            "current points list at " + i + "th iteration: " + currentPoints);
      }
  ...
  private void addLastEdgeConstraint(Matrix a, IDirectPosition pt,
      int lineIndex) {
  ...
  private void addAngleConstraint(Matrix a, IDirectPosition prevPt,
    ...
    double a1 = (prevPt.getY() - pt.getY())
        / (Math.pow(pt.getX() - prevPt.getX(), 2.0)
            + Math.pow(pt.getY() - prevPt.getY(), 2.0));
    double a2 = (pt.getX() - prevPt.getX())
        / (Math.pow(pt.getX() - prevPt.getX(), 2.0)
            + Math.pow(pt.getY() - prevPt.getY(), 2.0));
    double b1 = (nextPt.getY() - pt.getY())
        / (Math.pow(nextPt.getX() - pt.getX(), 2.0)
            + Math.pow(nextPt.getY() - pt.getY(), 2.0))
        + (pt.getY() - prevPt.getY())
            / (Math.pow(prevPt.getX() - pt.getX(), 2.0)
                + Math.pow(pt.getY() - prevPt.getY(), 2.0));
    double b2 = (pt.getX() - nextPt.getX())
        ...
        - (pt.getX() - prevPt.getX())
            / (Math.pow(pt.getX() - prevPt.getX(), 2.0)
                + Math.pow(prevPt.getY() - pt.getY(), 2.0));
    double c1 = (pt.getY() - nextPt.getY())
        / ((Math.pow(pt.getX() - nextPt.getX(), 2.0)
            + Math.pow(nextPt.getY() - pt.getY(), 2.0)));
    double c2 = (nextPt.getX() - pt.getX())
        / (Math.pow(nextPt.getX() - pt.getX(), 2.0)
            + Math.pow(pt.getY() - nextPt.getY(), 2.0));
  ...
  public Matrix getY() {
    ...
    for (int i = 1; i < currentPoints.size() - 1; i++) {
      ...
      double angleIni = Math
          .atan((iniNextPt.getY() - iniPt.getY())
              / (iniNextPt.getX() - iniPt.getX()))
          - Math.atan((iniPt.getY() - iniPrevPt.getY())
              / (iniPt.getX() - iniPrevPt.getX()));

      double angleFin = Math
          .atan((finalNextPt.getY() - finalPt.getY())
      ...
      IDirectPosition nextPt = currentPoints.get(i + 1);
      // compute the observation for the angle preservation
      double currObsAngle = Math
          .atan((nextPt.getY() - pt.getY()) / (nextPt.getX() - pt.getX()))
          - Math
      ...
      double lengthIni = Math
          .sqrt(Math.pow(iniPt.getX() - iniPrevPt.getX(), 2.0)
              + Math.pow(iniPt.getY() - iniPrevPt.getY(), 2.0));
      double lengthFin = Math
          .sqrt(Math.pow(finalPt.getX() - finalPrevPt.getX(), 2.0)
              + Math.pow(finalPt.getY() - finalPrevPt.getY(), 2.0));
      double obsLength = lengthIni * (1 - currentTime)
          + currentTime * lengthFin;
      // set the observation in the matrix
       ...
       * 
       * // finally, the last observation for the current line IDirectPosition
       * pt = currentPoints.get(currentPoints.size() - 1); IDirectPosition
       * prevPt = currentPoints.get(currentPoints.size() - 2);
       * 
