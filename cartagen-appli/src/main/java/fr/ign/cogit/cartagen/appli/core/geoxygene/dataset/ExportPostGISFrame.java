...
public class ExportPostGISFrame extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().equals("export")) {
            ...
            String url = "jdbc:postgresql://" + txtHost.getText() + ":" + txtPort.getText() + "/" + txtDb.getText();
            try {
                DriverManager.getConnection(url, txtUser.getText(), txtPwd.getText());
                JOptionPane.showMessageDialog(null, "Connection to " + url + " established");
    ...
    public <Feature extends IFeature> void write(String layerName, IFeatureCollection<IFeature> featurePop,
        ...
        try {
            ...
            File xmlFile = new File(
                    this.getClass().getClassLoader().getResource("xml/mapping_export_postgis.xml").getPath());
            ExportPostGISMappingXMLParser mappingXMLParser = new ExportPostGISMappingXMLParser(xmlFile);
            Hashtable<String, ArrayList<ArrayList<String>>> mappingPops = mappingXMLParser.parsePostGISMapping();
            // get the current pop mapping
            ArrayList<ArrayList<String>> mappingCurrentPop = mappingPops.get(layerName);
            ...
            ContentFeatureStore featureStore = (ContentFeatureStore) dataStore.getFeatureSource(featureTypeName);
            ...
            for (IFeature feature : featurePop) {
                ...
                for (int j = 0; j < mappingCurrentPop.size(); j++) {
                    ...
                    // get type
                    String getter = "get" + javaAttr.substring(0, 1).toUpperCase() + javaAttr.substring(1);
                    try {
                        ...
                        if (m.invoke(feature) == null)
                            ...
                            if (!m.invoke(feature).getClass().getName().equals(attrType))
                                liste.add(m.invoke(feature).toString());
                            else
                                liste.add(m.invoke(feature));
                        }
                    } catch (NoSuchMethodException e) {
                        String getterbis = "is" + javaAttr.substring(0, 1).toUpperCase() + javaAttr.substring(1);
                        try {
                            Method mbis = feature.getClass().getMethod(getterbis);
                            String attrType = mapping.get(2);
                            if (mbis.invoke(feature) == null)
                                liste.add(null);
                            else {
                                if (!mbis.invoke(feature).getClass().getName().equals(attrType))
                            ...
                            try {
                                Method mter = feature.getClass().getMethod(javaAttr);
                                liste.add(mter.invoke(feature));
                            } catch (NoSuchMethodException eter) {
                                logger.log(Level.SEVERE, "this method does not exist : " + mapping.get(1)
                                        + "on the layer : " + layerName);
                                // skip this attribute
                                liste.add(null);
                            }
                        }
            ...
            logger.log(Level.SEVERE, I18N.getString("ExportPostGISFrame.ErrorWritingFile") //$NON-NLS-1$
                    + tableName);
            e.printStackTrace();
        } catch (SchemaException e) {
            logger.log(Level.SEVERE, I18N.getString("ExportPostGISFrame.SchemeUsedForWritingFile") //$NON-NLS-1$
