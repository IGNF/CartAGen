...
public class LeastSquaresComponent extends JMenu implements MouseListener, ChangeListener {
    ...
    private JCheckBoxMenuItem chkVectorCaptureMode = new JCheckBoxMenuItem("Displacement vector capture Mode");
    ...
    public static LeastSquaresComponent getInstance() {
        if (LeastSquaresComponent.instance == null) {
            LeastSquaresComponent.instance = new LeastSquaresComponent("Least Squares");
        }
    ...
    class TestsAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (IFeature obj : SelectionUtil.getSelectedObjects(application)) {
                ...
                if (geom instanceof IPolygon)
                    objSegments.addAll(Segment.getSegmentList((IPolygon) geom, geom.coord().get(0)));
            ...
            Triangulation tri = new Triangulation(points, segments, new TriangulationSegmentFactoryImpl(),
                    new TriangulationTriangleFactoryImpl());
            tri.compute(true, application.getMainFrame().getSelectedProjectFrame().getLayerViewPanel().getViewport()
                    .getEnvelopeInModelCoordinates().getGeom(), options);
        ...
        public TestsAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Miscellaneous testing on Least Squares");
    ...
    class LSGeneAction extends AbstractAction {
        ...
        public LSGeneAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Least Squares generalisation with the complete configuration");
    ...
    class ConflateRoadsAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            try {
                ...
                process.execute(CartAGenDoc.getInstance().getCurrentDataset().getCartAGenDB());
            } catch (Exception e1) {
                e1.printStackTrace();
            }

            // enrich the road network
            NetworkEnrichment.enrichNetwork(dataset, dataset.getRoadNetwork(), false,
            ...
            MapspecsLS mapspecs = new MapspecsLS(Legend.getSYMBOLISATI0N_SCALE(), new HashSet<IFeature>(),
                    new HashSet<String>(), new HashSet<String>(), contraintesMalleables, contraintesExternes,
                    new HashSet<String>(), new HashSet<String>(), classesMalleables, poidsContraintes);
            mapspecs.getSelectedObjects().addAll(SelectionUtil
                    .getWindowObjects(CartAGenPlugin.getInstance().getApplication(), CartAGenDataSet.ROADS_POP));
            mapspecs.setDensStep(25.0);
            mapspecs.setFilter(true);
            mapspecs.setFilterThreshold(0.5);
            // puis on construit un scheduler
            sched = new ConflationScheduler(mapspecs, loadedVectors, LSVectorDisplConstraint1.class);
            ...
            NetworkEnrichment.destroyTopology(CartAGenDoc.getInstance().getCurrentDataset().getRoadNetwork());
        }
    ...
    class RubberConflationAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            for (DefaultFeature feat : loadedVectors) {
                Vector2D vect = new Vector2D(feat.getGeom().coord().get(0), feat.getGeom().coord().get(1));
                vectors.add(new ConflationVector(feat.getGeom().coord().get(0), vect));
            }
            RubberSheetingConflation rubber = new RubberSheetingConflation(
                    CartAGenDoc.getInstance().getCurrentDataset().getRoadNetwork().getSections(), vectors);
            rubber.conflation();
    ...
    class ExportConflationAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            try {
                ...
                specs += AdapterFactory.toJTSGeometryType(ILineString.class).getSimpleName();
        ...
        public ExportConflationAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Export conflated roads in a shapefile");
    ...
    class ShowConflationVectorsAction extends AbstractAction {
        ...
        public ShowConflationVectorsAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Display conflation vectors in the geometry pool");
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (IFeature point : sched.getPoints()) {
                if (((LSPoint) point).isFixed())
                    pool.addFeatureToGeometryPool(point.getGeom(), Color.PINK, 2);
                else if (((LSPoint) point).isCrossing())
                    pool.addFeatureToGeometryPool(point.getGeom(), Color.BLUE, 2);
                else
                    pool.addFeatureToGeometryPool(point.getGeom(), Color.RED, 2);
            }
            ...
            for (int i = 0; i < sched.getSystemeGlobal().getColumnNumber(); i += 2) {
                LSConstraint constr = sched.getSystemeGlobal().getConstraints().get(i);
                if (constr instanceof LSCrossingConstraint) {
                    System.out.println("résidu: " + sched.getSystemeGlobal().getResiduals().get(i) + "  obs: "
                            + sched.getSystemeGlobal().getObsVector().get(i));
                }
                if (constr instanceof LSVectorDisplConstraint) {
                    System.out.println("résidu conflation: " + sched.getSystemeGlobal().getResiduals().get(i)
                            + "  obs: " + sched.getSystemeGlobal().getObsVector().get(i));
                }
            }
        }

        public ShowLSPointsAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Display least square points in the geometry pool");
    ...
    class ShowRubberVectorAtPtAction extends AbstractAction {
        ...
        @Override
        public void actionPerformed(ActionEvent e) {
            String xS = JOptionPane.showInputDialog("x coordinate of the point");
            String yS = JOptionPane.showInputDialog("y coordinate of the point");
    ...
    class LSDiffusionAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            IRoadLine selectedRoad = (IRoadLine) SelectionUtil
                    .getSelectedObjects(CartAGenPlugin.getInstance().getApplication()).iterator().next();
            ILineString newGeom = CommonAlgorithms.translation(selectedRoad.getGeom(), 3.0, 3.0);
            ...
            CartAGenPlugin.getInstance().getApplication().getMainFrame().getSelectedProjectFrame().getLayerViewPanel()
                    .validate();
        }
    ...
    class LSSingleDiffusionAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            IFeature selected = SelectionUtil.getSelectedObjects(CartAGenPlugin.getInstance().getApplication())
                    .iterator().next();
            ...
            int vertex = CommonAlgorithmsFromCartAGen.getNearestVertexPositionFromPoint(selected.getGeom(),
                    vectorPoints.get(0));
            ...
            application.getMainFrame().getSelectedProjectFrame().getLayerViewPanel().validate();
        }
    ...
    class GenSelectedAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (IFeature feat : SelectionUtil.getSelectedObjects(application)) {
                CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool().addFeatureToGeometryPool(feat.getGeom(),
                        Color.GREEN, 2);
            ...
            MapspecsLS mapspecs;
            try {
                mapspecs = new MapspecsLS(new File("src/main/resources/xml/ls_default_mapspecs.xml"), selectedObjects);
    ...
    public void mouseClicked(MouseEvent e) {
        if (SwingUtilities.isLeftMouseButton(e) && e.getClickCount() == 2) {
            ...
            try {
                IDirectPosition pos = CartAGenPlugin.getInstance().getApplication().getMainFrame()
                        .getSelectedProjectFrame().getLayerViewPanel().getViewport().toModelDirectPosition(point);
