...
import fr.ign.cogit.cartagen.osm.lodharmonisation.process.AlignTreesAlongRoads;
...
public class AlignTreeAlongRoadPanel extends HarmonisationPanel {
  ...
  private IFeatureCollection<IFeature> trees, roads;
  private JSpinner spinDist, spinTreeWidth, spinOffset;

  public AlignTreeAlongRoadPanel() {
    super();
    this.trees = new FT_FeatureCollection<IFeature>();
    this.roads = new FT_FeatureCollection<IFeature>();
    ...
    detectionPanel.add(
        new JLabel(I18N.getString("AlignTreeAlongRoadPanel.minDistThresh")));
    detectionPanel.add(spinDist);
    detectionPanel.add(Box.createHorizontalGlue());
    detectionPanel
        .add(new JLabel(I18N.getString("AlignTreeAlongRoadPanel.treeWidth")));
  ...
  public Set<IGeneObj> triggerHarmonisation(boolean window,
    ...
    TreeAlongRoad detectionProc = new TreeAlongRoad(roads, trees,
        lodSlider.getValue(), minDist, treeWidth, offset);
