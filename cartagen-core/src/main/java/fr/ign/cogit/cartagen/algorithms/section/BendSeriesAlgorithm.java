...
public class BendSeriesAlgorithm {
    ...
    public BendSeriesAlgorithm(BendSeries bendSeries, double innerWidth,
        ...
        this.pool = CartAGenDoc.getInstance().getCurrentDataset()
                .getGeometryPool();
    }
    ...
    /**
     * Implementation of the bend series schematisation algorithm (Lecordix et
     * al 1997).
     * 
     * @return
     */
    public ILineString schematisation() {
        ...
        if (bendSeries.getBends().size() > 3) {
            ...
            bendsToKeep.add(bendSeries.getBends()
                    .get(bendSeries.getBends().size() - 1));
        }
                ...
                if (bendSeries.getBends().size() < 11)
                    bendIndices.put(bend,
                            (bend.getShapeMeasure() / shapeTotal
                                    + bend.getSizeMeasure() / sizeTotal
                                    + bend.getLength() / lengthTotal
                                    + bend.getSymmetry()) / 4.0);
                else
                    bendIndices.put(bend,
                            (bend.getShapeMeasure() / shapeTotal
        ...
        if (bendSeries.getBends().size() > 4) {
            ...
            bendsToKeep
                    .add(bendsToRemoveSize.get(bendsToRemoveSize.size() - 1));
            bendsToRemoveIndex.remove(
                    bendsToRemoveSize.get(bendsToRemoveSize.size() - 1));
        }

        // ***********************************************************************
        // now, choose the bends to remove (two consecutive bends) according to
        // height, width, length and symmetry of bends (summarized in the
        // index).
        Bend bend1 = null, bend2 = null;
        int bend1Index = 0, bend2Index = 0;
        // first, if there are not enough bends, do nothing (schematisation is
        // not
        // the good algorithm)
        ...
        if (bendsToRemoveIndex.size() == 2) {
            ...
            for (int j = 1; j < bendsToRemoveIndex.size(); j++) {
                ...
                if (currentBendNb == bendSeries.getBends().indexOf(b1) + 1
                        || currentBendNb == bendSeries.getBends().indexOf(b1)
                                - 1) {
                    bend1 = b1;
        ...
        if (debugMode) {
            if (bend1 != null) {
                ...
                pool.addFeatureToGeometryPool(bend1.getGeom(), Color.ORANGE, 1);
            }
            if (bend2 != null) {
                System.out.println("bend2 index: " + bend2Index);
                System.out.println(bend2.getGeom());
                pool.addFeatureToGeometryPool(bend2.getGeom(), Color.MAGENTA,
                        1);
            }
        }
        // *************************************************
        ...
        // first, cut the bend series into two parts, by removing the chosen
        // bends
        ...
        if (debugMode) {
            for (IDirectPosition pt : part1)
                pool.addFeatureToGeometryPool(pt.toGM_Point(), Color.GREEN, 1);
            for (IDirectPosition pt : part2)
                pool.addFeatureToGeometryPool(pt.toGM_Point(), Color.BLUE, 1);
        }

        // compute the joining point. It's the middle of part1 end point and
        // part2
        ...
        Segment joiningSeg = new Segment(part1.get(part1.size() - 1),
                part2.get(0));
        IDirectPosition joiningPt = joiningSeg
                .getWeightedMiddlePoint(lengthRatio);
        ...
        for (int i = part1.size() - 2; i >= 0; i--) {
            ...
            if (lastHalfBend.contains(pt)) {
                Vector2D vect = vect1Main.changeNorm(
                        vect1Main.norme() * (1 - totalLength / halfBendLength));
                proj = vect.translate(proj);
            }

            // cushion in the perpendicular direction
            Vector2D vect = vect1Perp.changeNorm(
                    vect1Perp.norme() * (1 - totalLength / part1Length));
            proj = vect.translate(proj);
        ...
        for (int i = 1; i < part2.size() - 1; i++) {
            ...
            if (firstHalfBend.contains(pt)) {
                Vector2D vect = vect2Main.changeNorm(
                        vect2Main.norme() * (1 - totalLength / halfBendLength));
                proj = vect.translate(proj);
            }

            // cushion in the perpendicular direction
            Vector2D vect = vect2Perp.changeNorm(
                    vect2Perp.norme() * (1 - totalLength / part2Length));
            proj = vect.translate(proj);
    ...
    public ILineString accordion() {
        ...
        // first, get the middle of the bend series (the generalised line will
        // be
        ...
        if (debugMode)
            System.out.println(
                    bendSeries.getBends().size() + " bends in the series");
        for (Bend bend : bendSeries.getBends()) {
            ...
            for (int i = nbPt - 1; i >= 0; i--) {
                ...
                distortionVector.scalarMultiplication(
                        (lineLength - totalLength) / lineLength);
                generalisedBend.add(0, distortionVector.translate(pt));
            }
            genBendsMap.put(bend, generalisedBend);
            if (debugMode)
                pool.addFeatureToGeometryPool(
                        new GM_LineString(generalisedBend), Color.GREEN, 2);
        }
        ...
        // ************************************************************************************
        // finally, translate the generalised line to the initial middle point
        // of
        // the line
     ...
     * finding the orthogonal projection to bend orientation of the first point
     * of the bend. If no such point can be found, the search is tried with the
    ...
    private Vector2D findVectorForAccordion(Bend bend) {
        ...
        if (debugMode)
            System.out.println(
                    "bend orientation: " + orientation * 180 / Math.PI);
        ...
        if (debugMode)
            System.out.println(
                    "vector orientation: " + angle.getValeur() * 180 / Math.PI);
        ...
        IGeometry intersection = segment
                .intersection(new GM_LineString(coordList));
    ...
    /**
     * Put the algorithm in debug mode, it displays intermediate geometries in
     * the geometry pool.
     */
