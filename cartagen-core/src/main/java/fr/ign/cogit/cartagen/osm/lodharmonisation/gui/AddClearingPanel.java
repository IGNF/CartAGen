package fr.ign.cogit.cartagen.osm.lodharmonisation.gui;
...
import fr.ign.cogit.cartagen.osm.lodharmonisation.process.AddClearingInForest;
import fr.ign.cogit.cartagen.osm.lodharmonisation.process.AddClearingInForest.ClearingShape;
...
public class AddClearingPanel extends HarmonisationPanel {
  ...
  private IFeatureCollection<IFeature> buildings, blocks, forests;

  public AddClearingPanel() {
    super();
    this.buildings = new FT_FeatureCollection<IFeature>();
    this.blocks = new FT_FeatureCollection<IFeature>();
    this.forests = new FT_FeatureCollection<IFeature>();
    ...
    detectionPanel
        .add(new JLabel(I18N.getString("AddClearingPanel.overlapThresh")));
    detectionPanel.add(spinOverlap);
    detectionPanel
        .add(new JLabel(I18N.getString("AddClearingPanel.radiusThresh")));
    detectionPanel.add(spinRadius);
    detectionPanel
        .add(new JLabel(I18N.getString("AddClearingPanel.bigBuildThresh")));
    ...
    shapeCombo = new JComboBox(
        new String[] { "BUFFER", "CONVEX", "RECTANGLE" });
  ...
  private void fillCollections(boolean window, Set<IGeneObj> windowObjs) {
    if (window) {
      ...
      buildings
          .addAll(CartAGenDoc.getInstance().getCurrentDataset().getBuildings());
    ...
    for (Set<IFeature> cluster : clusters.getClusters().keySet()) {
      if (cluster.size() < 4) {
        double totalArea = 0.0;
        for (IFeature obj : cluster)
      ...
      blocks.add(new UrbanBlock(clusters.getClusters().get(cluster), null, null,
          buildingGroup, null));
    }
  }
}
