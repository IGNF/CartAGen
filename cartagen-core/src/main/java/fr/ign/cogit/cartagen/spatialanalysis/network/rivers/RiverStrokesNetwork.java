...
public class RiverStrokesNetwork extends StrokesNetwork {
	...
	private static Logger logger = Logger.getLogger(RiverStrokesNetwork.class.getName());
	...
	public void findSimpleIslands(double areaThreshold) {
		...
		for (Face face : carteTopo.getListeFaces()) {
			if (face.getGeom().area() < areaThreshold) {
				...
				this.getSimpleIslands().add(new RiverIsland((IPolygon) face.getGeom(), outline));
			}
		}
	}
	...
	/**
	 * Build the river strokes of {@code this} network using the method from (Touya,
	 * 2007).
	 */
	public void buildRiverStrokes() {
		...
		for (NoeudHydrographique source : this.getSources()) {
			...
			TronconHydrographique section = (TronconHydrographique) source.getArcsSortants().iterator().next();
		...
		while (!downstreamNodes.isEmpty()) {
			...
			if (node.getArcsSortants().size() == 1) {
				...
				if (!downstreamNodes.contains(downstreamSection.getNoeudFinal())) {
					downstreamNodes.add((NoeudHydrographique) downstreamSection.getNoeudFinal());
				}
				...
				this.strahlerOrders.put(downstreamSection, this.computeStrahlerAtConfluence(orders));
				...
				if (node.getArcsEntrants().size() == 1) {
					...
					Collection<ArcReseau> remainingBranches = new HashSet<ArcReseau>(node.getArcsSortants());
					...
					for (ArcReseau branch : remainingBranches) {
						...
						if (!downstreamNodes.contains(branch.getNoeudFinal())) {
							downstreamNodes.add(0, (NoeudHydrographique) branch.getNoeudFinal());
						}
						// compute Strahler orders
	...
	private ArcReseau manageBraidedConfluence(NoeudHydrographique node, ArcReseau upStream,
		...
		RiverStroke upstreamStroke = this.getUpstreamStrokes(node).iterator().next();
	...
	/**
	 * First, decision is based on river name. If there is no name preferrence, the
	 * length of upstream stroke makes the decision (the longest continues). If
	 * lengthes are close (less than 25% difference), the angle is used to make the
	 * decision.
	 * 
	 * @param node
	 ...
	 * @return
	 */
	private RiverStroke makeDecisionAtConfluence(NoeudHydrographique node, TronconHydrographique downstreamSection,
	...
	private RiverStroke getMainNonBraidedStroke(Set<RiverStroke> upstreamStrokes) {
		double maxLength = 0;
		RiverStroke unbraided = null;
		for (RiverStroke stroke : upstreamStrokes) {
			if (stroke.isBraided()) {
	...
	/**
	 * Correct the geometry direction of what should be the source outflow stream at
	 * nodes that have two downstream sections and no upstream section. The section
	 * that is modified is the one that is connected only at one side. Sources and
	 * sinks must have been identified prior to this method.
	 */
	public void correctDoubleSources() {
		for (ArcReseau arc : this.getFeatures()) {
			if (this.getSinks().contains(arc.getNoeudFinal())) {
				...
				NoeudHydrographique doubleNode = (NoeudHydrographique) arc.getNoeudInitial();
				...
				arc.setGeom(GeometryEngine.getFactory().createILineString(arc.getGeom().coord().reverse()));
