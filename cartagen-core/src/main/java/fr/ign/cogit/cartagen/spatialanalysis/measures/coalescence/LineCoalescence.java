...
public class LineCoalescence {
   ...
   */
  private ArrayList<CoalescenceConflictType> coalescenceTypes = null;

  /**
   * @return
   */
  public ArrayList<CoalescenceConflictType> getCoalescenceTypes() {
  ...
  public void compute() {
    ...
    if (!(geomStart instanceof ILineString)) {
      if (logger.isTraceEnabled()) {
        logger.trace(
            "Immpossible to perform Coalescence detection - non linear geometry");
      }
      return;
    }
    if (geomStart.coord().size() < 3) {
      if (logger.isTraceEnabled()) {
        logger.trace(
            "Impossible to perform Coalescence detection - not enough points on geometry");
      }
      return;
    }
    ...
    this.coalescenceTypes = new ArrayList<CoalescenceConflictType>();

    // For each point of the line
    List<IDirectPosition> positionsForCurrentLine = new ArrayList<IDirectPosition>();
    CoalescenceConflictType coalescenceType = CoalescenceConflictType.NONE;
    for (IDirectPosition directPosition : geom.coord()) {
      ...
      if (logger.isTraceEnabled()) {
        ...
        if (!(offset1.isEmpty())) {
          logger.trace(
              "Distance at left: " + nearestPoint1.distance(directPosition));
        }
        if (!(offset2.isEmpty())) {
          logger.trace(
              "Distance at right: " + nearestPoint2.distance(directPosition));
        }
      }
      ...
      if (offset1.isEmpty()
        ...
        coa = true;
        if (coalescenceType == CoalescenceConflictType.NONE) {
          addNewLineString(positionsForCurrentLine, directPosition, minDistance,
              coalescenceType);

          positionsForCurrentLine = new ArrayList<IDirectPosition>();
          coalescenceType = CoalescenceConflictType.LEFT;
        } else if (coalescenceType == CoalescenceConflictType.RIGHT) {
          coalescenceType = CoalescenceConflictType.BOTH;
        }
      } else {
        if (debug && !offset1.isEmpty())
      ...
      if (offset2.isEmpty()
        ...
        if (coalescenceType == CoalescenceConflictType.NONE) {
          ...
          positionsForCurrentLine = new ArrayList<IDirectPosition>();
          coalescenceType = CoalescenceConflictType.RIGHT;
        } else if (coalescenceType == CoalescenceConflictType.LEFT) {
          coalescenceType = CoalescenceConflictType.BOTH;
        }
      } else {
        if (debug && !offset2.isEmpty())
      ...
      if (!coa) {
        if (coalescenceType != CoalescenceConflictType.NONE) {
          ...
          positionsForCurrentLine = new ArrayList<IDirectPosition>();
          coalescenceType = CoalescenceConflictType.NONE;
        }
      }
  ...
  private void fusionLineString(double distanceMin) {
    ...
    CoalescenceConflictType previousPreviousType = this.getCoalescenceTypes()
        .get(0);

    ILineString previousLine = this.getSections().get(1);
    CoalescenceConflictType previousType = this.getCoalescenceTypes().get(1);

    for (int i = 2; i < this.getNbSections(); i++) {
      ILineString line = this.getSections().get(i);
      CoalescenceConflictType type = this.getCoalescenceTypes().get(i);

      if (previousPreviousType != CoalescenceConflictType.NONE
          && type != CoalescenceConflictType.NONE && previousPreviousLine
              .endPoint().distance(line.startPoint()) < distanceMin) {
        previousPreviousLine
            .removeControlPoint(previousPreviousLine.sizeControlPoint() - 1);
        ...
        this.getCoalescenceTypes()
            .remove(this.getCoalescenceTypes().size() - 1);
        if (previousPreviousType != type
            || previousType != CoalescenceConflictType.NONE
                && (previousType != previousPreviousType
                    || previousType != type)) {
          this.getCoalescenceTypes()
              .remove(this.getCoalescenceTypes().size() - 1);
          this.getCoalescenceTypes().add(CoalescenceConflictType.BOTH);
          previousPreviousType = CoalescenceConflictType.BOTH;
        }
        this.nbSections -= 2;
  ...
  private void addNewLineString(List<IDirectPosition> positionsForCurrentLine,
      IDirectPosition directPosition, double minDistance,
      CoalescenceConflictType coalescenceType) {
