 ...
 * 
 */
public class CoalescenceFunction extends ELECTRECriterion
    implements LegibilityFunction {
  ...
  public double getValue(IGeometry feature, double scale) {
    ...
    for (int i = 0; i < measure.getSections().size(); i++) {
      if (measure.getCoalescenceTypes().get(i) == CoalescenceConflictType.NONE)
        continue;

      ILineString section = measure.getSections().get(i);
      if (measure.getCoalescenceTypes().get(i) == CoalescenceConflictType.LEFT
          || measure.getCoalescenceTypes()
              .get(i) == CoalescenceConflictType.RIGHT)
        coalescenceAmount += section.length();
      else if (measure.getCoalescenceTypes()
          .get(i) == CoalescenceConflictType.BOTH
          || measure.getCoalescenceTypes()
              .get(i) == CoalescenceConflictType.HETEROG)
