...
public class ForkCrossRoad extends SimpleCrossRoad {
    ...
    public static boolean isForkNode(NoeudRoutier node, double forkAngle,
        ...
        for (int i = 0; i < 3; i++) { // (i) = the supposed major road
            ...
            double angleSym1 = Math.PI - Math.abs(CommonAlgorithmsFromCartAGen
                    .angleBetween2Lines((GM_LineString) forkRoad1.getGeom(),
                            (GM_LineString) majorRoad.getGeom()));
            double angleSym2 = Math.PI - Math.abs(CommonAlgorithmsFromCartAGen
                    .angleBetween2Lines((GM_LineString) forkRoad2.getGeom(),
                            (GM_LineString) majorRoad.getGeom()));

            // test if the difference between the 2 angles is bigger than
            // threshold
    ...
    public ForkCrossRoad(NoeudRoutier node, double forkAngle,
            double symmAngle) {
        super(node.getGeom());
        this.setNode(node);
        // set the degree
        ...
        for (int i = 0; i < 3; i++) { // (i) = la principale supposée
            ...
            // compute the fork angle
            this.forkAngle = Math
                    .abs(CommonAlgorithmsFromCartAGen.angleBetween2Lines(
