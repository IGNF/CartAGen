...
import fr.ign.cogit.cartagen.appli.utilities.DisplayLayerTableModel;
import fr.ign.cogit.cartagen.appli.utilities.JColorSelectionButton;
import fr.ign.cogit.cartagen.appli.utilities.renderer.ColorRenderer;
...
public class GeometryPoolMenu extends JMenu {
    ...
    /**
     */
    public JCheckBoxMenuItem mGeomPoolVisible = new JCheckBoxMenuItem(new GeomPoolVisibleAction());
    /**
     */
    public JMenuItem mGeomPoolManagement = new JMenuItem(new GeomPoolManagementAction());
    /**
     */
    private JMenuItem mGeomPoolEmpty = new JMenuItem(new GeomPoolEmptyAction());
    /**
     */
    private JMenuItem mGeomPoolAddObjects = new JMenuItem(new GeomPoolAddObjectsAction());

    private JMenuItem mGeomPoolAddDeleted = new JMenuItem(new GeomPoolAddDeletedAction());
    /**
     */
    public JCheckBoxMenuItem mGeomPoolDrawSegments = new JCheckBoxMenuItem("Draw segments");
    ...
    class GeomPoolManagementAction extends AbstractAction {
        ...
        public GeomPoolManagementAction() {
            putValue(Action.SHORT_DESCRIPTION, "Show the Geometry Pool management frame");
        ...
        public void actionPerformed(ActionEvent arg0) {
            if (CartAGenDoc.getInstance().getCurrentDataset() == null) {
                if (geometryPool == null)
                    geometryPool = new GeometryPool(DataSet.getInstance(), CartAGenPlugin.getInstance().getApplication()
                            .getMainFrame().getSelectedProjectFrame().getSld());
                for (IFeature feat : CartAGenPlugin.getInstance().getApplication().getMainFrame()
    ...
    class AddBufferAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            for (IFeature feat : CartAGenPlugin.getInstance().getApplication().getMainFrame().getSelectedProjectFrame()
    ...
    class AddBufferEtcFrame extends JFrame implements ActionListener {
        ...
        private JCheckBox chkBuffer, chkHalfR, chkHalfL, chkCentroid, chkOffsetR, chkOffsetL, chkMbr, chkConvex;
        private JColorSelectionButton colorBuffer, colorHalfR, colorHalfL, colorCentroid, colorOffsetR, colorOffsetL,
                colorMbr, colorConvex;
        private JSpinner spinBuffer, spinHalfR, spinHalfL, spinOffsetR, spinOffsetL, spinBufferWidth, spinHalfRWidth,
                spinHalfLWidth, spinOffsetRWidth, spinOffsetLWidth, spinMbrWidth, spinConvexWidth;
        ...
        private void drawToGeomPool() {
            ...
            for (IGeneObj obj : objects) {
                if (chkBuffer.isSelected()) {
                    IGeometry buffer = obj.getGeom().buffer((Double) spinBuffer.getValue());
                    CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool().addFeatureToGeometryPool(buffer,
                            colorBuffer.getColor(), (Integer) spinBufferWidth.getValue());
                }
                if (chkHalfR.isSelected()) {
                    if (obj.getGeom() instanceof ILineString) {
                        IGeometry buffer = BufferComputing.buildLineHalfBuffer((ILineString) obj.getGeom(),
                                (Double) spinHalfR.getValue(), Side.RIGHT);

                        CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool().addFeatureToGeometryPool(buffer,
                                colorHalfR.getColor(), (Integer) spinHalfRWidth.getValue());
                    }
                }
                if (chkHalfL.isSelected()) {
                    if (obj.getGeom() instanceof ILineString) {
                        IGeometry buffer = BufferComputing.buildLineHalfBuffer((ILineString) obj.getGeom(),
                                (Double) spinHalfL.getValue(), Side.LEFT);
                        CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool().addFeatureToGeometryPool(buffer,
                                colorHalfL.getColor(), (Integer) spinHalfLWidth.getValue());
                    }
                }
                if (chkOffsetR.isSelected()) {
                    if (obj.getGeom() instanceof ILineString) {
                        IGeometry buffer = BufferComputing.buildHalfOffsetLine(Side.RIGHT, (ILineString) obj.getGeom(),
                                (Double) spinOffsetR.getValue());
                        CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool().addFeatureToGeometryPool(buffer,
                                colorOffsetR.getColor(), (Integer) spinOffsetRWidth.getValue());
                    }
                }
                if (chkOffsetL.isSelected()) {
                    if (obj.getGeom() instanceof ILineString) {
                        IGeometry buffer = BufferComputing.buildHalfOffsetLine(Side.LEFT, (ILineString) obj.getGeom(),
                                (Double) spinOffsetL.getValue());
                        CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool().addFeatureToGeometryPool(buffer,
                                colorOffsetL.getColor(), (Integer) spinOffsetLWidth.getValue());
                    }
                }
                if (chkCentroid.isSelected()) {
                    IGeometry centroid = obj.getGeom().centroid().toGM_Point();
                    CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool().addFeatureToGeometryPool(centroid,
                            colorCentroid.getColor(), 1);
                }
                if (chkMbr.isSelected()) {
                    IGeometry mbr = SmallestSurroundingRectangleComputation.getSSR(obj.getGeom());
                    CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool().addFeatureToGeometryPool(mbr,
                            colorMbr.getColor(), (Integer) spinMbrWidth.getValue());
                }
                if (chkConvex.isSelected()) {
                    IGeometry hull = obj.getGeom().convexHull();
                    CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool().addFeatureToGeometryPool(hull,
                            colorConvex.getColor(), (Integer) spinConvexWidth.getValue());
                }
            }
        }
    }
    ...
    class GeomPoolEmptyAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent arg0) {
            if (CartAGenDoc.getInstance().getCurrentDataset() != null)
                CartAGenDoc.getInstance().getCurrentDataset().getGeometryPoolPop().clear();
            else
                application.getMainFrame().getSelectedProjectFrame().getLayer("Geometry Pool").getFeatureCollection()
                        .clear();
        }
    ...
    class GeomPoolAddDeletedAction extends AbstractAction {
        ...
        public GeomPoolAddDeletedAction() {
            putValue(Action.SHORT_DESCRIPTION, "Add the geometries of deleted objects to the geometry pool");
    ...
    class GeomPoolAddDeletedFrame extends JFrame implements ActionListener {
        ...
        private void displayGeoms() {
            if (CartAGenDoc.getInstance().getCurrentDataset() == null) {
                ...
                CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool().setSld(CartAGenPlugin.getInstance()
                        .getApplication().getMainFrame().getSelectedProjectFrame().getSld());
                geometryPool = CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool();
            }
                ...
                if (display) {
                    for (IFeature feat : layer.getFeatureCollection()) {
                        if (feat.isDeleted()) {
                            ...
                            geometryPool.addFeatureToGeometryPool(feat.getGeom(), color, width);
                        }
                    }
                }
            }
    ...
    class GeomPoolVisibleAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            if (geomPoolLayer != null) {
                ...
                ProjectFrame frame = CartAGenPlugin.getInstance().getApplication().getMainFrame()
                        .getSelectedProjectFrame();
                if (mGeomPoolVisible.isSelected()) {
                    if (DataSet.getInstance().getPopulation(geomPoolLayer.getName()) == null) {
                        geomPoolLayer.getSld().getDataSet()
                                .addPopulation(new Population<IFeature>(geomPoolLayer.getName()));
                    }
        ...
        public GeomPoolVisibleAction() {
            putValue(Action.SHORT_DESCRIPTION, "Make the Geometry Pool visible or not");
