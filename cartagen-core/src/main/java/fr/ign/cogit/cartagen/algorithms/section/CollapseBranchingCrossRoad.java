...
public class CollapseBranchingCrossRoad {
  ...
  public IRoadNode collapseToPoint() {
    ...
    Vector2D direction = new Vector2D(
        branching.getMinorRoadExtern().getGeom().coord().get(1), cornerMinor);
    if (!branching.getMinorRoadExtern().getGeom().coord().get(0)
        .equals(cornerMinor))
      direction = new Vector2D(
          branching.getMinorRoadExtern().getGeom().coord()
    ...
    if (!branching.getGeom().exteriorLineString()
      ...
      proj = CommonAlgorithms.getNearestPoint(
          branching.getGeom().exteriorLineString(), proj.toGM_Point());
    }
    ...
    for (IRoadLine road : this.branching.getInternalRoads()) {
      if (branching.getMainRoadIntern().contains(road)) {
        if (road.getGeom().intersects(node.getGeom())) {
          ...
          ILineString[] lines = CommonAlgorithmsFromCartAGen
              .splitLine(road.getGeom(), proj, 0.001);
