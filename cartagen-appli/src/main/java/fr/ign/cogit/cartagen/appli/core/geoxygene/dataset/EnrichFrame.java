...
public class EnrichFrame extends JDialog implements ActionListener {
    ...
    private int version = 1;// version 1 pour l'ancien chargeur, version 2 pour
                            // le
    ...
    public void initFrame() {
        ...
        this.enrichBuildings = new JCheckBox("Building enrichment: towns & blocks", false);
        this.enrichBuildings.setMnemonic(KeyEvent.VK_B);
        this.enrichBuildings.setSelected(true);

        this.enrichBuildingsAlign = new JCheckBox("Building enrichment: urban alignments", false);
        ...
        checkPanel.setBorder(BorderFactory.createTitledBorder("Please select the needed enrichments"));
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == this.ok) {
            ...
            if (this.enrichRoads.isSelected()) {
                NetworkEnrichment.enrichNetwork(this.dataSet, this.dataSet.getRoadNetwork(), this.factory);
                this.addEnrichmentToFrame(CartAGenDataSet.ROAD_NODES_POP, IPoint.class);

            }

            if (this.enrichHydro.isSelected()) {
                NetworkEnrichment.enrichNetwork(this.dataSet, this.dataSet.getHydroNetwork(), this.factory);
            ...
            if (this.enrichBuildings.isSelected()) {
                ...
                if (enrichissementBatiAlign) {
                    ...
                    this.addEnrichmentToFrame(CartAGenDataSet.URBAN_ALIGNMENTS_POP, ILineString.class);
                }
            }

            if (this.buildNetworkFaces.isSelected()) {
                NetworkEnrichment.buildNetworkFaces(this.dataSet, this.factory);
                this.addEnrichmentToFrame(CartAGenDataSet.NETWORK_FACES_POP, IPolygon.class);
            }
            this.setVisible(false);
        }
    }

    private void addEnrichmentToFrame(String populationName, Class<? extends IGeometry> geometryType) {
        ...
        fts.getRules().add(
                LayerFactory.createRule(geometryType, fillColor.darker(), fillColor, opacity, opacity, strokeWidth));
