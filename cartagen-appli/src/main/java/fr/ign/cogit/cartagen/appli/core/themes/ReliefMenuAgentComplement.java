...
public class ReliefMenuAgentComplement {
    ...
    private static Logger logger = Logger.getLogger(ReliefMenuAgentComplement.class.getName());

    private JLabel lblAgent = new JLabel("          AGENT");

    private JMenuItem mReliefEnrichissement = new JMenuItem(new EnrichReliefAction());
    ...
    private JMenuItem mReliefActiverPoints = new JMenuItem(new PtAgentsActivateAction());
    public JCheckBoxMenuItem mSegmentsReliefVoir = new JCheckBoxMenuItem("Display segments");
    public JCheckBoxMenuItem mPointsReliefVoir = new JCheckBoxMenuItem("Display points");
    public JCheckBoxMenuItem mReliefSegmentsVoirTexteDistance = new JCheckBoxMenuItem("Display distance text");
    ...
    private class EnrichReliefAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            ((ReliefFieldAgent) AgentUtil
                    .getAgentFromGeneObj(CartAGenDoc.getInstance().getCurrentDataset().getReliefField())).enrich();
        }
    ...
    private class LoadReliefAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            ReliefMenuAgentComplement.logger.info("Chargement de "
                    + AgentUtil.getAgentFromGeneObj(CartAGenDoc.getInstance().getCurrentDataset().getReliefField()));
            AgentGeneralisationScheduler.getInstance()
                    .add(AgentUtil.getAgentFromGeneObj(CartAGenDoc.getInstance().getCurrentDataset().getReliefField()));
        }
    ...
    private class LoadPtsAction extends AbstractAction {
        ...
        public LoadPtsAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Load unbalanced points in the relief agent");
    ...
    private class DisplayInfoAction extends AbstractAction {
        ...
        public DisplayInfoAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Display information on the relief agent");
    ...
    private class PtAgentsActivateAction extends AbstractAction {
        ...
        @Override
        public void actionPerformed(ActionEvent e) {
            ReliefMenuAgentComplement.logger.severe("activation des agents point du champ relief");
            for (IPointAgent ap : ((ReliefFieldAgent) AgentUtil
                    .getAgentFromGeneObj(CartAGenDoc.getInstance().getCurrentDataset().getReliefField()))
                            .getPointAgents()) {
                new Thread(ap).start();
            }
            ReliefMenuAgentComplement.logger.severe("fin activation des agents point du champ relief");
        }

        public PtAgentsActivateAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Activate the point agents of the relief agent");
