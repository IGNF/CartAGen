...
import fr.ign.cogit.cartagen.osm.lodharmonisation.process.ExtendBuiltUpAreas;
...
public class ExtendBuiltUpPanel extends HarmonisationPanel {
  ...
  private IFeatureCollection<IFeature> buildings, builtUps;

  public ExtendBuiltUpPanel() {
    super();
    this.buildings = new FT_FeatureCollection<IFeature>();
    this.builtUps = new FT_FeatureCollection<IFeature>();
    ...
    detectionPanel
        .add(new JLabel(I18N.getString("ExtendBuiltUpPanel.distanceThresh")));
    ...
    if (window) {
      for (IGeneObj obj : windowObjs) {
        ...
        if (obj instanceof ISimpleLandUseArea) {
          if (((ISimpleLandUseArea) obj)
              .getType() == OsmLandUseTypology.RESIDENTIAL.ordinal())
            builtUps.add(obj);
        }
      }
    } else {
