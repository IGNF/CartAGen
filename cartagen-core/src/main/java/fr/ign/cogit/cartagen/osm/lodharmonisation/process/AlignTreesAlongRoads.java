  ...
  public Set<IGeneObj> harmonise() {
    ...
    for (LoDSpatialRelation relation : leftInconsistencies) {
      if (!trees.contains(relation.getFeature2()))
        trees.add((IGeneObj) relation.getFeature2());
      else
    ...
    for (LoDSpatialRelation relation : leftInconsistencies) {
      ...
      IDirectPosition newTree = CommonAlgorithms.getNearestPoint(leftBufferLine,
          tree.getGeom());
    ...
    for (LoDSpatialRelation relation : rightInconsistencies) {
      ...
      IDirectPosition newTree = CommonAlgorithms
          .getNearestPoint(rightBufferLine, tree.getGeom());
      tree.setGeom(newTree.toGM_Point());
      modifiedFeats.add(tree);
    }
    ...
    for (ITreePoint tree : multipleAlign.keySet()) {
      ...
      if (offsets.size() != 2) {
        ...
        IDirectPosition newTree = CommonAlgorithms
            .getNearestPoint(offsets.iterator().next(), tree.getGeom());
        tree.setGeom(newTree.toGM_Point());
        modifiedFeats.add(tree);
      } else {
