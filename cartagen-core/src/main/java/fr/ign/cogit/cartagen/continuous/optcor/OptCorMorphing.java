...
public class OptCorMorphing implements ContinuousGeneralisationMethod {
  ...
  private List<SubLineCorrespondance> matchLinePoints(
    ...
    while (!finished) {
      ...
      if (type.equals(CorrespondanceType.C1)) {
        IDirectPosition prevPt = subLinesFin.get(currentPair.getV() - 1).coord()
        ...
        mapping.add(0,
            new C1SubLineCorrespondance(subLinesIni.get(currentPair.getU() - 1),
                subLinesFin.get(currentPair.getV() - 1).endPoint(), prevPt,
                nextPt));
      } else if (type.equals(CorrespondanceType.C1_)) {
        IDirectPosition prevPt = subLinesIni.get(currentPair.getU() - 1).coord()
        ...
        mapping.add(0,
            new C1InvSubLineCorrespondance(
                subLinesIni.get(currentPair.getU() - 1).endPoint(),
                subLinesFin.get(currentPair.getV() - 1), prevPt, nextPt));
      } else if (type.equals(CorrespondanceType.C2)) {
        mapping.add(0,
            new C2SubLineCorrespondance(subLinesIni.get(currentPair.getU() - 1),
                subLinesFin.get(currentPair.getV() - 1)));
        ...
        for (int i = Math.max(0, previousPair.getV()); i < currentPair
            .getV(); i++)
          finalLines.add(subLinesFin.get(i));
        mapping.add(0, new C3SubLineCorrespondance(
            subLinesIni.get(currentPair.getU() - 1), finalLines));
      } else {
        List<ILineString> initialLines = new ArrayList<>();
        Pair<Integer, Integer> previousPair = minimumStorage.get(currentPair);
        for (int i = Math.max(0, previousPair.getU()); i < currentPair
            .getU(); i++)
          initialLines.add(subLinesIni.get(i));
        mapping.add(0, new C3InvSubLineCorrespondance(
    ...
    if (!isSublineMapped(subLinesIni.get(0), mapping)) {
      mapping.add(0,
          new C1SubLineCorrespondance(subLinesIni.get(0),
              subLinesFin.get(0).startPoint(), null,
              subLinesFin.get(0).getControlPoint(1)));
    }
  ...
  private IBezier fitBezierToLine(ILineString line) {
    ...
    Vector2D vect2 = new Vector2D(line.endPoint(),
        line.coord().get(line.coord().size() - 2)).changeNorm(k);
