...
public class GridTessellation<T> {
    ...
    public GridTessellation(IEnvelope envelope, double width) {
        ...
        this.width = width;
        this.corner = new DirectPosition(envelope.center().getX() - (envelope.width() / 2),
    ...
    private void computeCells() {
        ...
        for (int i = 1; i <= rowNb; i++) {
            for (int j = 1; j <= colNb; j++) {
                ...
                this.cells.add(new GridCell<T>(this, i, j, new DirectPosition(xCenter, yCenter), width));
            }
        }
    }
    ...
    private void computeCells(T defaultValue) {
        ...
        for (int i = 1; i <= rowNb; i++) {
            for (int j = 1; j <= colNb; j++) {
                ...
                this.cells.add(new GridCell<T>(this, i, j, new DirectPosition(xCenter, yCenter), width, defaultValue));
            }
        }
    }
    ...
    public Set<GridCell<T>> getContainingCells(IDirectPosition point) {
        Set<GridCell<T>> set = new HashSet<>();
