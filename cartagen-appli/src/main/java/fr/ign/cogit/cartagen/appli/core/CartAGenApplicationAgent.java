 ...
 * @author GTouya
 */
public class CartAGenApplicationAgent extends GeOxygeneApplication
    implements AgentObserver {
  @SuppressWarnings("unused")
  private static final Logger logger = Logger
      .getLogger(CartAGenApplicationAgent.class);
  ...
  private CartAGenApplicationAgent() {
    ...
    AbstractGeometryEngine
        .setGeometrie(this.getProperties().getGeometryEngine());
    GeometryEngine.init();
  ...
  private void reorganizeMenus() {
    ...
    for (int i = 3; i < this.getMainFrame().getMenuBar().getComponentCount()
        - 1; i++) {
    ...
    for (JMenu menu : cartagenPlugins)
      this.getMainFrame().getMenuBar().add(menu,
          this.getMainFrame().getMenuBar().getMenuCount() - 2);
  ...
  public void addAgentModules() {
    ...
    GeneralisationMenuAgentComplement.getInstance()
        .add(this.getMainFrame().getMenuBar());
    ...
    GeneralisationRightPanelAgentComplement.getInstance().cAfficherAgentActif
        .setSelected(false);
  ...
  private JPanel getRightPanel() {
    for (Component comp : this.getMainFrame().getGui().getContentPane()
        .getComponents()) {
  ...
  public static void main(String[] args) {
    ...
    cartoappli.getMainFrame().getGui()
