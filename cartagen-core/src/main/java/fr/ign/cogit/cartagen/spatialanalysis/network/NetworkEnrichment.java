...
public class NetworkEnrichment {
  ...
  public static void agregerTronconsAnaloguesAdjacents(CartAGenDataSet dataset,
    ...
    while (ats != null) {
      ...
      IDirectPosition c00 = coords0.get(0),
          c0n = coords0.get(coords0.size() - 1);
      IDirectPosition c10 = coords1.get(0),
          c1n = coords1.get(coords1.size() - 1);
  ...
  public static void aggregateAnalogAdjacentSections(CartAGenDataSet dataset,
    ...
    for (INetworkNode node : net.getNodes()) {
      ...
      IPoint section0FinalPoint = new GM_Point(sections.get(0).getGeom().coord()
          .get(sections.get(0).getGeom().coord().size() - 1));
      IPoint section1InitialPoint = new GM_Point(
          sections.get(1).getGeom().coord().get(0));
      IPoint section1FinalPoint = new GM_Point(sections.get(1).getGeom().coord()
          .get(sections.get(1).getGeom().coord().size() - 1));
      ...
      sections.get(0)
          .setInitialGeom((ILineString) sections.get(0).getGeom().clone());
