...
public class LeastSquaresComponent extends JMenu
  ...
  class TestsAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      GeometryPool pool = CartAGenDoc.getInstance().getCurrentDataset()
          .getGeometryPool();
      pool.setSld(sld);
      String options = "pczeBQ"; // on convertit les entrées dans le format
                                 // pivot
       ...
       * diff.setDefaultMapspecs(); MapspecsLS mapspecs = diff.getMapspecs();
       * try { mapspecs.saveToXml(new File("d:\\ls_default_mapspecs.xml")); }
       * catch (TransformerException | IOException e1) { e1.printStackTrace(); }
       */
    }
  ...
  class ConflateRoadsAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent arg0) {
      ...
      MapspecsLS mapspecs = new MapspecsLS(Legend.getSYMBOLISATI0N_SCALE(),
          new HashSet<IFeature>(), new HashSet<String>(), new HashSet<String>(),
          contraintesMalleables, contraintesExternes, new HashSet<String>(),
          new HashSet<String>(), classesMalleables, poidsContraintes);
      mapspecs.getSelectedObjects().addAll(
          fr.ign.cogit.geoxygene.appli.plugin.cartagen.selection.SelectionUtil
              .getWindowObjects(CartAGenPlugin.getInstance().getApplication(),
      ...
      NetworkEnrichment.destroyTopology(
  ...
  class RubberConflationAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent arg0) {
      ...
      RubberSheetingConflation rubber = new RubberSheetingConflation(CartAGenDoc
          .getInstance().getCurrentDataset().getRoadNetwork().getSections(),
          vectors);
      rubber.conflation();
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      for (int i = 0; i < sched.getSystemeGlobal().getColumnNumber(); i += 2) {
        ...
        if (constr instanceof LSCrossingConstraint) {
          System.out.println(
              "résidu: " + sched.getSystemeGlobal().getResiduals().get(i)
        ...
        if (constr instanceof LSVectorDisplConstraint) {
          System.out.println("résidu conflation: "
              + sched.getSystemeGlobal().getResiduals().get(i) + "  obs: "
  ...
  class LSDiffusionAction extends AbstractAction {
    ...
    @Override
    public void actionPerformed(ActionEvent e) {
      IRoadLine selectedRoad = (IRoadLine) fr.ign.cogit.geoxygene.appli.plugin.cartagen.selection.SelectionUtil
          .getSelectedObjects(CartAGenPlugin.getInstance().getApplication())
          .iterator().next();
      ILineString newGeom = CommonAlgorithms.translation(selectedRoad.getGeom(),
          3.0, 3.0);
  ...
  class LSSingleDiffusionAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      IFeature selected = fr.ign.cogit.geoxygene.appli.plugin.cartagen.selection.SelectionUtil
          .getSelectedObjects(CartAGenPlugin.getInstance().getApplication())
          .iterator().next();
      ...
      application.getMainFrame().getSelectedProjectFrame().getLayerViewPanel()
          .validate();
    }
  ...
  class GenSelectedAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      for (IFeature feat : SelectionUtil.getSelectedObjects(application)) {
        CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool()
            .addFeatureToGeometryPool(feat.getGeom(), Color.GREEN, 2);
      ...
      MapspecsLS mapspecs;
      try {
        mapspecs = new MapspecsLS(
            new File("src/main/resources/xml/ls_default_mapspecs.xml"),
            selectedObjects);
  ...
  public void mouseClicked(MouseEvent e) {
    if (SwingUtilities.isLeftMouseButton(e) && e.getClickCount() == 2) {
      ...
      try {
        IDirectPosition pos = CartAGenPlugin.getInstance().getApplication()
