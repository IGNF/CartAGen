...
import fr.ign.cogit.cartagen.osm.lodharmonisation.process.AdjustFunctionalSitesBounds;
...
public class AdjustFunctionalSitePanel extends HarmonisationPanel {
  ...
  private IFeatureCollection<IFeature> schools, hospitals;
  private IFeatureCollection<IGeneObj> buildings, sportsFields, roads;

  public AdjustFunctionalSitePanel() {
    super();
    this.buildings = new FT_FeatureCollection<IGeneObj>();
    this.schools = new FT_FeatureCollection<IFeature>();
    this.hospitals = new FT_FeatureCollection<IFeature>();
    ...
    detectionPanel.add(
        new JLabel(I18N.getString("AdjustFunctionalSitePanel.belongThresh")));
    detectionPanel.add(spinBelongSch);
    detectionPanel.add(
        new JLabel(I18N.getString("AdjustFunctionalSitePanel.excludeThresh")));
  ...
  public Set<IGeneObj> triggerHarmonisation(boolean window,
    ...
    if (chkSchools.isSelected()) {
      IFeatureCollection<IFeature> components = new FT_FeatureCollection<IFeature>();
  ...
  private void fillCollections(boolean window, Set<IGeneObj> windowObjs,
    ...
    if (window) {
      ...
      if (hospital) {
        ...
        hospitals
            .addAll(((OsmDataset) CartAGenDoc.getInstance().getCurrentDataset())
                .getHospitals());
      }
      if (school) {
        sportsFields.addAll(
            CartAGenDoc.getInstance().getCurrentDataset().getSportsFields());
        schools
            .addAll(((OsmDataset) CartAGenDoc.getInstance().getCurrentDataset())
                .getSchools());
      }
