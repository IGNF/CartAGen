...
public class RoadLineImpl extends TronconDeRouteImpl {
    ...
    private static Logger logger = Logger
            .getLogger(RoadLineImpl.class.getName());
    // Public fields //
    ...
    public RoadLineImpl(Reseau res, String nature, ICurve geom, int importance,
        ...
        super(res, false, geom, importance);
        this.setNature(nature);
    ...
    public Object getAttribute(String nom) {
        ...
        if (attribute.getMemberName().equals("geom")) {
            RoadLineImpl.logger.warning(
                    "WARNING : Pour récupérer la primitive géométrique par défaut, veuillez utiliser "
        ...
        if (attribute.getMemberName().equals("topo")) {
            RoadLineImpl.logger.warning(
                    "WARNING : Pour récupérer la primitive topologique par défaut, veuillez utiliser "
        ...
        if (attribute.getNomField().length() == 0) {
            ...
            nomFieldMaj = Character
                    .toUpperCase(attribute.getNomField().charAt(0))
        ...
        while (!classe.equals(Object.class)) {
            try {
                Method methodGetter = classe
                        .getDeclaredMethod(nomGetFieldMethod, (Class[]) null);
                valeur = methodGetter.invoke(this, (Object[]) null);
                return valeur;
            } catch (SecurityException e) {
                RoadLineImpl.logger.config(
                        "SecurityException pendant l'appel de la méthode "
                                + nomGetFieldMethod + " sur la classe "
                                + classe);
                ...
                // logger.trace("La mï¿½thode "+nomGetFieldMethod+" n'existe pas
                // dans la
                // classe "+classe);
            } catch (IllegalAccessException e) {
                RoadLineImpl.logger.config(
                        "IllegalAccessException pendant l'appel de la méthode "
                                + nomGetFieldMethod + " sur la classe "
                                + classe);
    ...
    public void setAttribute(String nom, Object value) {
        ...
        if (attribute.getMemberName().equals("geom")) {
            RoadLineImpl.logger.warning(
                    "WARNING : Pour affecter la primitive géométrique par défaut, veuillez utiliser "
                            + "la méthode FT_Feature.getGeom() et non pas MdFeature.getAttribute(AttributeType attribute)");
            this.setGeom((IGeometry) value);
        } else if (attribute.getMemberName().equals("topo")) {
            RoadLineImpl.logger.warning(
                    "WARNING : Pour affecter la primitive topologique par défaut, veuillez utiliser "
                            + "la méthode FT_Feature.getTopo() et non pas MdFeature.getAttribute(AttributeType attribute)");
            ...
            try {
                ...
                if (attribute.getNomField().length() == 0) {
                    ...
                    nomFieldMaj2 = Character
                            .toUpperCase(attribute.getNomField().charAt(0))
                            + attribute.getNomField().substring(1);
                }
                String nomSetFieldMethod = "set" + nomFieldMaj2;
                Method methodSetter = this.getClass()
                        .getDeclaredMethod(nomSetFieldMethod, value.getClass());
    ...
    public RoadLineImpl clone() {
        return new RoadLineImpl(this.getReseau(), false,
                (ICurve) this.getGeom().clone());
    }
