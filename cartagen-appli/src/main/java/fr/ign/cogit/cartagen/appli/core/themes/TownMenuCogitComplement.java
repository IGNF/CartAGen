...
public class TownMenuCogitComplement extends JMenu {
    ...
    public JMenuItem mSuppressionImpasses = new JMenuItem(new DeleteDeadEndsAction());
    private JMenuItem mAggrBlocks = new JMenuItem(new AggregateBlocksAction());
    private JMenuItem mTownStreetSelection = new JMenuItem(new StreetSelectionAction());
    ...
    private class DeleteDeadEndsAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (IFeature sel : selection) {
                ...
                EliminateTownDeadEnds algo = new EliminateTownDeadEnds(town.getDeadEnds(), town.getStreetNetwork(),
    ...
    private class AggregateBlocksAction extends AbstractAction {
        ...
        @Override
        public void actionPerformed(ActionEvent e) {
            List<IFeature> sel = SelectionUtil.getListOfSelectedObjects(menu.getApplication());
            if (sel.size() != 2) {
                JOptionPane.showMessageDialog(menu.getApplication().getMainFrame().getGui(),
                        "Please select two city blocks");
                return;
            }
            if (!(sel.get(0) instanceof UrbanBlock) || !(sel.get(1) instanceof UrbanBlock)) {
                JOptionPane.showMessageDialog(menu.getApplication().getMainFrame().getGui(),
    ...
    private class StreetSelectionAction extends AbstractAction {
        ...
        @Override
        public void actionPerformed(ActionEvent e) {
            Set<IFeature> sel = SelectionUtil.getSelectedObjects(menu.getApplication());
