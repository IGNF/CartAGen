...
public class UrbanAlignmentMenuCogitComplement extends JMenu {
    ...
    private Logger logger = Logger.getLogger(UrbanAlignmentMenuCogitComplement.class.getName());

    private UrbanAlignmentMenu menu;

    private JLabel lblCogit = new JLabel("          COGIT");
    private JMenuItem mCreateAlignmentInBlocks = new JMenuItem(new CreateAlignmentsInBlocksAction());
    private JMenuItem mCreateAlignmentFromBuildings = new JMenuItem(new CreateAlignmentsFromBuildingsAction());
    private JMenuItem mDestroyAlignments = new JMenuItem(new DestroyAlignmentsInBlocksAction());
    private JMenuItem mProjectionsAlignmentsDetection = new JMenuItem(new DetectAlignmentsByProjectionsAction());
    private JMenuItem mMSTAlignmentsDetection = new JMenuItem(new DetectMSTAlignmentsAction());
    private JMenuItem mDeleteSmallestBuilding = new JMenuItem(new DeleteSmallestBuildingAction());
    private JMenuItem mEnsureSpatialRepartition = new JMenuItem(new EnsureSpatialReparitionAction());
    private JMenuItem mCorrectOrientations = new JMenuItem(new CorrectOrientationsAction());
    ...
    private class CreateAlignmentsInBlocksAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            Thread th = new Thread(new Runnable() {
                @Override
                public void run() {
                    for (IFeature sel : SelectionUtil.getSelectedObjects(menu.getApplication())) {
                        if (sel.isDeleted()) {
                            continue;
                        }
                        if (sel instanceof IUrbanBlock) {
                            UrbanEnrichment.createUrbanAlignmentsBasedOnSections((IUrbanBlock) sel,
                                    CartAGenDoc.getInstance().getCurrentDataset(),
                                    CartAGenPlugin.getInstance().getCurrentGeneObjImpl().getCreationFactory());
                        }
                    }
                }
            });
        ...
        public CreateAlignmentsInBlocksAction() {
            super();
            this.putValue(Action.NAME, "Automated detection of alignments in selected blocks");
        }
    ...
    private class CreateAlignmentsFromBuildingsAction extends AbstractAction {
                ...
                public void run() {
                    ...
                    for (IFeature sel : SelectionUtil
                        ...
                        if (sel instanceof IUrbanElement) {
                            buildAgents.add((IUrbanElementAgent) AgentUtil.getAgentFromGeneObj((IUrbanElement) sel));
                        }
                    }
                    new UrbanAlignmentAgent(CartAGenPlugin.getInstance().getCurrentGeneObjImpl().getCreationFactory()
                            .createUrbanAlignment(), buildAgents);
                }
            });
            th.start();
        }
        ...
        public CreateAlignmentsFromBuildingsAction() {
            super();
            this.putValue(Action.NAME, "Alignment creation based on selected buildings");
        }
    ...
    private class DetectAlignmentsByProjectionsAction extends AbstractAction {
        ...
        public DetectAlignmentsByProjectionsAction() {
            super();
            this.putValue(Action.NAME, "Detection of straight alignments based on projections (beta)");
        }
    ...
    private class DetectMSTAlignmentsAction extends AbstractAction {
            ...
            Thread th = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    for (IFeature sel : SelectionUtil
                        ...
                        ArrayList<MinSpanningTree> trees = MinSpanningTree.buildMinSpanningTree(coll, 500);
                        if (trees == null) {
                            continue;
                        }
        ...
        public DetectMSTAlignmentsAction() {
            super();
            this.putValue(Action.NAME, "Detection of curvilinear alignments based on MST (beta)");
        }
    ...
    private class EnsureSpatialReparitionAction extends AbstractAction {
        ...
        public EnsureSpatialReparitionAction() {
            super();
            this.putValue(Action.NAME, "Homogeneous spatial repartition of buildings");
        }
