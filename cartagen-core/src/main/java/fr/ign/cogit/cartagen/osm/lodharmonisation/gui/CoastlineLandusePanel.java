...
import fr.ign.cogit.cartagen.osm.lodharmonisation.process.RemoveLandFromSea;
...
public class CoastlineLandusePanel extends HarmonisationPanel {
  ...
  private IFeatureCollection<IFeature> coastlines, landuse;

  public CoastlineLandusePanel() {
    super();
    this.coastlines = new FT_FeatureCollection<IFeature>();
    this.landuse = new FT_FeatureCollection<IFeature>();
  ...
  private void fillCollections(boolean window, Set<IGeneObj> windowObjs) {
    if (window) {
      ...
      coastlines.addAll(
          CartAGenDoc.getInstance().getCurrentDataset().getCoastlines());
      landuse.addAll(
