  ...
  public void compute(boolean isPersistantObject) {
    ...
    if (!(geomStart instanceof ILineString)) {
      if (LineGaussianSmoothing.logger.isInfoEnabled()) {
        LineGaussianSmoothing.logger.info(
            "Impossible to perform Gaussian smoothing - non linear geometry");
      }
      return;
    }
    if (geomStart.coord().size() < 3) {
      if (LineGaussianSmoothing.logger.isInfoEnabled()) {
        LineGaussianSmoothing.logger.info(
            "Impossible to perform Gaussian smoothing - not enough points on geometry");
      }
      return;
    }
    ...
    ILineString geomResult = LineGaussianSmoothing
        .lineGaussianSmoothing(geoxLine, this.sigma, this.resolution);
    ...
    if (geomResult != null) {
      ...
      if (LineGaussianSmoothing.logger.isInfoEnabled()) {
        LineGaussianSmoothing.logger
            .info("Gaussian smoothing performed on " + this.geneObj);
      }
    }
