 ...
 * 
 */
package fr.ign.cogit.cartagen.appli.geompool;
...
public class GeomPoolFrame extends JFrame {
    ...
    private void initComponents() {
        ...
        GeomPoolFrame.pnlBuildings = new themeTabPanel(CartAGenDataSet.BUILDINGS_POP);
        GeomPoolFrame.pnlHydroLines = new themeTabPanel(CartAGenDataSet.WATER_LINES_POP);
        GeomPoolFrame.pnlHydroSurfaces = new themeTabPanel(CartAGenDataSet.WATER_AREAS_POP);
        GeomPoolFrame.pnlRailwayLines = new themeTabPanel(CartAGenDataSet.RAILWAY_LINES_POP);
        GeomPoolFrame.pnlElectricityLines = new themeTabPanel(CartAGenDataSet.ELECTRICITY_LINES_POP);
        GeomPoolFrame.pnlContourLines = new themeTabPanel(CartAGenDataSet.CONTOUR_LINES_POP);
        GeomPoolFrame.pnlHydroLines = new themeTabPanel(CartAGenDataSet.WATER_LINES_POP);
        GeomPoolFrame.pnlLandUseAreas = new themeTabPanel(CartAGenDataSet.LANDUSE_AREAS_POP);
        GeomPoolFrame.pnlAdminFields = new themeTabPanel(CartAGenDataSet.ADMIN_FIELDS_POP);
        GeomPoolFrame.pnlHydroLines = new themeTabPanel(CartAGenDataSet.WATER_LINES_POP);
        GeomPoolFrame.pnlRoadStrokes = new themeTabPanel(CartAGenDataSet.ROAD_STROKES_POP);
        GeomPoolFrame.pnlRoundabouts = new themeTabPanel(CartAGenDataSet.ROUNDABOUTS_POP);
        GeomPoolFrame.pnlBranchings = new themeTabPanel(CartAGenDataSet.BRANCHINGS_POP);

        // Initial themes tabs
        GeomPoolFrame.pnlRoadsInitial = new themeTabPanel(CartAGenDataSet.ROADS_POP);
        GeomPoolFrame.pnlBuildingsInitial = new themeTabPanel(CartAGenDataSet.BUILDINGS_POP);
        GeomPoolFrame.pnlHydroLinesInitial = new themeTabPanel(CartAGenDataSet.WATER_LINES_POP);
        GeomPoolFrame.pnlHydroSurfacesInitial = new themeTabPanel(CartAGenDataSet.WATER_AREAS_POP);
        GeomPoolFrame.pnlRailwayLinesInitial = new themeTabPanel(CartAGenDataSet.RAILWAY_LINES_POP);
        GeomPoolFrame.pnlElectricityLinesInitial = new themeTabPanel(CartAGenDataSet.ELECTRICITY_LINES_POP);
        GeomPoolFrame.pnlContourLinesInitial = new themeTabPanel(CartAGenDataSet.CONTOUR_LINES_POP);
        GeomPoolFrame.pnlHydroLinesInitial = new themeTabPanel(CartAGenDataSet.WATER_LINES_POP);
        GeomPoolFrame.pnlLandUseAreasInitial = new themeTabPanel(CartAGenDataSet.LANDUSE_AREAS_POP);
        GeomPoolFrame.pnlAdminFieldsInitial = new themeTabPanel(CartAGenDataSet.ADMIN_FIELDS_POP);
        GeomPoolFrame.pnlHydroLinesInitial = new themeTabPanel(CartAGenDataSet.WATER_LINES_POP);
        GeomPoolFrame.pnlRoadStrokesInitial = new themeTabPanel(CartAGenDataSet.ROAD_STROKES_POP);
        GeomPoolFrame.pnlRoundaboutsInitial = new themeTabPanel(CartAGenDataSet.ROUNDABOUTS_POP);
        GeomPoolFrame.pnlBranchingsInitial = new themeTabPanel(CartAGenDataSet.BRANCHINGS_POP);
    ...
    private void placeComponents() {
        ...
        GeomPoolFrame.themes.addTab("Surfacic rivers", GeomPoolFrame.pnlHydroSurfaces);
        GeomPoolFrame.themes.addTab("Railway", GeomPoolFrame.pnlRailwayLines);
        GeomPoolFrame.themes.addTab("Electricity", GeomPoolFrame.pnlElectricityLines);
        ...
        GeomPoolFrame.themesInitial.addTab("Buildings", GeomPoolFrame.pnlBuildingsInitial);
        GeomPoolFrame.themesInitial.addTab("Linear rivers", GeomPoolFrame.pnlHydroLinesInitial);
        GeomPoolFrame.themesInitial.addTab("Surfacic rivers", GeomPoolFrame.pnlHydroSurfacesInitial);
        GeomPoolFrame.themesInitial.addTab("Railway", GeomPoolFrame.pnlRailwayLinesInitial);
        GeomPoolFrame.themesInitial.addTab("Electricity", GeomPoolFrame.pnlElectricityLinesInitial);
        GeomPoolFrame.themesInitial.addTab("Contour lines", GeomPoolFrame.pnlContourLinesInitial);
        GeomPoolFrame.themesInitial.addTab("Land use", GeomPoolFrame.pnlLandUseAreasInitial);
        GeomPoolFrame.themesInitial.addTab("Admin fields", GeomPoolFrame.pnlAdminFieldsInitial);
        GeomPoolFrame.themesInitial.addTab("Strokes", GeomPoolFrame.pnlRoadStrokesInitial);
        GeomPoolFrame.themesInitial.addTab("Roundabouts", GeomPoolFrame.pnlRoundaboutsInitial);
        GeomPoolFrame.themesInitial.addTab("Branchings", GeomPoolFrame.pnlBranchingsInitial);

        // Tabbed pane for both datasets
        GeomPoolFrame.dataSets.addTab("Current dataset", GeomPoolFrame.themes);
        GeomPoolFrame.dataSets.addTab("Initial dataset", GeomPoolFrame.themesInitial);
    ...
    private static void drawGeomPool() {
        ...
        CartAGenPlugin.getInstance().getApplication().getMainFrame().getSelectedProjectFrame().getLayer("Geometry Pool")
        ...
        for (int i = 0; i < GeomPoolFrame.themes.getComponentCount(); i++) {
            ...
            for (IGeneObj obj : CartAGenDoc.getInstance().getCurrentDataset().getCartagenPop(tab.popName)) {
                ...
                // Unchanged features
                if (tab.tabUnchanged.getSelectedColumn() != 0 && !obj.hasBeenCreated() && !obj.isDeleted()
                        && obj.getGeom().equals(obj.getInitialGeom())) {
                    GeomPoolFrame.drawGeom(tab.tabUnchanged, obj.getGeom());
                }

                // Modified features
                if (tab.tabModified.getSelectedColumn() != 0 && !obj.hasBeenCreated() && !obj.isDeleted()
                        && !obj.getGeom().equals(obj.getInitialGeom())) {
                    GeomPoolFrame.drawGeom(tab.tabModified, obj.getGeom());
                }

                // Created features
                if (tab.tabCreated.getSelectedColumn() != 0 && obj.hasBeenCreated() && !obj.isDeleted()) {
                    GeomPoolFrame.drawGeom(tab.tabCreated, obj.getGeom());
                }

                // Created features
                if (tab.tabDeleted.getSelectedColumn() != 0 && !obj.hasBeenCreated() && obj.isDeleted()) {
        ...
        for (int i = 0; i < GeomPoolFrame.themesInitial.getComponentCount(); i++) {
            ...
            // Treatment of each tab for each theme
            themeTabPanel tab = (themeTabPanel) GeomPoolFrame.themesInitial.getComponent(i);
            for (IGeneObj obj : CartAGenDoc.getInstance().getInitialDataset().getCartagenPop(tab.popName)) {
    ...
    private static void drawGeom(JTable tab, IGeometry geom) {
        Color col = ((ColourTableModel) tab.getModel()).getValueAt(1, tab.getSelectedColumn());
        IGeometry gm;
        ...
        CartAGenDoc.getInstance().getCurrentDataset().getGeometryPool().addFeatureToGeometryPool(gm, col, 2);
    }
