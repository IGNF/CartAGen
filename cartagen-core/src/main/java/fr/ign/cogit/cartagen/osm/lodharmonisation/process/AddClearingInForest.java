...
import fr.ign.cogit.cartagen.osm.schema.OSMSchemaFactory;
    ...
    for (LoDSpatialRelation instance : inconsistencies) {
      ...
      if (!instance.getFeature2().getGeom().contains(geomClearing)) {
        ...
        if (newGeom instanceof IPolygon)
          ...
          if (instance.getFeature2() instanceof IGeneObj)
            ((IGeneObj) instance.getFeature2()).eliminate();
        }
      }
        ...
        if (geomClearing instanceof IPolygon) {
          ...
          IPolygon simplePol = CommonAlgorithmsFromCartAGen
              .getBiggerFromMultiSurface(
                  (IMultiSurface<IOrientableSurface>) geomClearing);
          ((IPolygon) instance.getFeature2().getGeom())
              .addInterior(simplePol.getExterior());
        }
      }
      modifiedFeats.add((IGeneObj) instance.getFeature2());
    }
