...
public class UrbanAGENTProcess extends GeneralisationProcess
    implements StoppableProcess {

  private boolean generaliseRoads = true;
  private boolean blockGrayingAllowed = true;
  private AgentObserver observer;

  public UrbanAGENTProcess(Conductor chefO, AgentObserver observer) {
    super(chefO);
  ...
  protected void triggerGeneralisation(GeographicSpace space) {
    ...
    GeographicAgentGeneralisation ago = AgentUtil.getAgentFromGeneObj(town);
    if (!generaliseRoads) {
      Constraint streetsConstraint = null;
      for (Constraint constraint : ago.getConstraints()) {
        if (constraint instanceof StreetDensity)
          streetsConstraint = constraint;
      }
      ago.getConstraints().remove(streetsConstraint);
    }
    if (!blockGrayingAllowed) {
      for (IBlockAgent ba : ((TownAgent) ago).getComponents())
        ((BlockAgent) ba).setGrayingNecessary(false);
    }
    AgentGeneralisationScheduler.getInstance().add(ago);
    AgentGeneralisationScheduler.getInstance().activate();
  }
  ...
  public boolean isGeneraliseRoads() {
    return generaliseRoads;
  }

  public void setGeneraliseRoads(boolean generaliseRoads) {
    this.generaliseRoads = generaliseRoads;
  }

  public boolean isBlockGrayingAllowed() {
    return blockGrayingAllowed;
  }

  public void setBlockGrayingAllowed(boolean blockGrayingAllowed) {
    this.blockGrayingAllowed = blockGrayingAllowed;
  }
}
