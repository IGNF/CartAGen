...
import fr.ign.cogit.cartagen.osm.lodharmonisation.operation.AdjustBoundsToComponents;
  ...
  public Set<IGeneObj> harmonise() {
    ...
    for (FunctionalSiteGroup group : groups) {
      ...
      for (LoDSpatialRelation relation : group.getToInclude())
        componentsToInclude.add(
            (IPolygon) ((IGeneObj) relation.getFeature2()).getSymbolGeom());
      for (LoDSpatialRelation relation : group.getToExclude())
        componentsToExclude.add(
            (IPolygon) ((IGeneObj) relation.getFeature2()).getSymbolGeom());
