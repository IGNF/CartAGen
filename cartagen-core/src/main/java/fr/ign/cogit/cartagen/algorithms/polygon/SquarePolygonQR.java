 ...
 * squares adjustment. The system solution uses QR Decomposition before
 * inverting A'PA, possibly more numerically stable
 * @author ILokhat
 * 
 */
public class SquarePolygonQR {
  ...
  private RealMatrix y, p, xCurrent;
  private double poidsPtfFixe = 5;
  private double poids90 = 100;
  private double poids0 = 50;
   ...
   * @param semiRightTol
   */
  public SquarePolygonQR(double rightTol, double flatTol, double semiRightTol) {
  ...
  public void setPolygon(IPolygon p) {
    ...
    for (int i = 0; i < nb_edges; ++i) {
      double vx = points.get((i + 1) % nb_edges).getX() - points.get(i).getX();
      double vy = points.get((i + 1) % nb_edges).getY() - points.get(i).getY();
      vecs[i] = (new Vector2D(vx, vy)).normalize();
    ...
    for (int i = 0; i < vecs.length; ++i) {
      double dot = vecs[i].dotProduct(vecs[(i + 1) % nb_edges]);
      double cross = Vector3D.crossProduct(new Vector3D(vecs[i].getX(), vecs[i].getY(), 0),
          new Vector3D((vecs[(i + 1) % nb_edges]).getX(), (vecs[(i + 1) % nb_edges]).getY(), 0)).getNorm();
      ...
      if (Math.abs(dot) <= rTol) {
        ...
        System.out.println((i + 1) + " angle droit : " + Vector2D.angle(vecs[i], (vecs[(i + 1) % nb_edges])));
      }
      // getting list of vertices with quasi flat angles
      else if (cross <= flatTol) {
        indicesFlat.add((i + 1) % nb_edges);
        System.out.println((i + 1) + " angle plat: " + Vector2D.angle(vecs[i], (vecs[(i + 1) % nb_edges])));
      }
      // getting list of vertices with quasi semi-right angles
      else if (dot <= hrTol1 && dot >= hrTol2) {
        indicesHrAig.add((i + 1) % nb_edges);
        System.out.println((i + 1) + " angle hr aig: " + Vector2D.angle(vecs[i], (vecs[(i + 1) % nb_edges])));
      } else if (dot >= -hrTol1 && dot <= -hrTol2) {
        indicesHrObt.add((i + 1) % nb_edges);
        System.out.println((i + 1) + " angle hr obt: " + Vector2D.angle(vecs[i], (vecs[(i + 1) % nb_edges])));
      }
    }
    // observations matrix Y
    this.y = getY();
    // X0
    this.xCurrent = MatrixUtils.createRealMatrix(y.getSubMatrix(0, 2 * nb_edges - 1, 0, 0).getData());
  ...
  private double dotProduct(int a, int b, int c) {
    double v1x = points.get(b).getX() - points.get(a).getX();
    double v1y = points.get(b).getY() - points.get(a).getY();
    double v2x = points.get(c).getX() - points.get(b).getX();
    double v2y = points.get(c).getY() - points.get(b).getY();
  ...
  private double crossProduct(int a, int b, int c) {
    ...
    double v2x = points.get(c).getX() - points.get(b).getX();
    double v2y = points.get(c).getY() - points.get(b).getY();
    Vector3D v1 = new Vector3D(v1x, v1y, 0);
    Vector3D v2 = new Vector3D(v2x, v2y, 0);
    return v1.crossProduct(v2).getZ();
  }

  private RealMatrix getIdentityMatrix() {
    return MatrixUtils.createRealIdentityMatrix(nb_edges * 2);
  }

  // creates the submatrix corresponding to the partial derivatives of the
  // scalar product
  private RealMatrix getDotProductSubMatrix(List<Integer> indices) {
    RealMatrix m = new Array2DRowRealMatrix();
    if (indices.size() > 0) {
      m = MatrixUtils.createRealMatrix(indices.size(), 2 * nb_edges);
  ...
  private RealMatrix getA() {
    ...
    RealMatrix id = getIdentityMatrix();
    RealMatrix scal = getDotProductSubMatrix(indicesRight);
    RealMatrix cross = getCrossProductSubMatrix();
    RealMatrix scalHr = getDotProductSubMatrix(indicesHrAig);
    RealMatrix scalHr2 = getDotProductSubMatrix(indicesHrObt);
    ...
    RealMatrix a = MatrixUtils.createRealMatrix(2 * nb_edges + scalNbRows + crosNbRows + scHr1NbRows + scHr2NbRows,
        2 * nb_edges);
    if ((id.getRowDimension() > 0))
      a.setSubMatrix(id.getData(), 0, 0);
    // a.setMatrix(0, 2 * nb_edges - 1, 0, 2 * nb_edges - 1, id);
    if (scalNbRows > 0)
      a.setSubMatrix(scal.getData(), 2 * nb_edges, 0);
    // a.setMatrix(2 * nb_edges, 2 * nb_edges + scalNbRows - 1, 0, 2 * nb_edges
    // - 1, scal);
    if ((crosNbRows > 0))
      a.setSubMatrix(cross.getData(), 2 * nb_edges + scalNbRows, 0);
    // a.setMatrix(2 * nb_edges + scalNbRows, 2 * nb_edges + scalNbRows +
    // crosNbRows - 1, 0, 2 * nb_edges - 1, cross);
    if (scHr1NbRows > 0)
      a.setSubMatrix(scalHr.getData(), 2 * nb_edges + scalNbRows + crosNbRows, 0);
    // a.setMatrix(2 * nb_edges + scalNbRows + crosNbRows, 2 * nb_edges +
    // scalNbRows + crosNbRows + scHr1NbRows - 1, 0,
    // 2 * nb_edges - 1, scalHr);
    if (scHr2NbRows > 0)
      a.setSubMatrix(scalHr2.getData(), 2 * nb_edges + scalNbRows + crosNbRows + scHr1NbRows, 0);
  ...
  private RealMatrix getY() {
    RealMatrix y = MatrixUtils.createRealMatrix(
        2 * nb_edges + indicesRight.size() + indicesFlat.size() + indicesHrAig.size() + indicesHrObt.size(), 1);

    for (int i = 0; i < nb_edges; ++i) {
      y.setEntry(2 * i, 0, points.get(i).getX());
      y.setEntry((2 * i) + 1, 0, points.get(i).getY());
    }
    for (int i = 0; i < indicesHrAig.size(); ++i)
      y.setEntry(2 * nb_edges + indicesRight.size() + indicesFlat.size() + i, 0, Math.cos(Math.PI / 4));
    for (int i = 0; i < indicesHrObt.size(); ++i)
      y.setEntry(2 * nb_edges + indicesRight.size() + indicesFlat.size() + indicesHrAig.size() + i, 0,
          Math.cos(Math.PI * 3 / 4));
    return y;
  }

  // B = Y - S(Xcourant)
  private RealMatrix getB() {
    RealMatrix s = MatrixUtils.createRealMatrix(
        2 * nb_edges + indicesRight.size() + indicesFlat.size() + indicesHrAig.size() + indicesHrObt.size(), 1);
    for (int i = 0; i < nb_edges; ++i) {
      s.setEntry(2 * i, 0, points.get(i).getX());
      s.setEntry((2 * i) + 1, 0, points.get(i).getY());
    }
    // F(i-1,i,i+1) (Xn-1 Xn).(Xn Xn+1)
    for (int i = 0; i < indicesRight.size(); ++i) {
      int[] pointsAround = getPointsAround(indicesRight.get(i));
      s.setEntry(2 * nb_edges + i, 0, dotProduct(pointsAround[0], indicesRight.get(i), pointsAround[1]));
    }
    // G(i-1,i,i+1) (Xn-1 Xn)^(Xn Xn+1)
    for (int i = 0; i < indicesFlat.size(); ++i) {
      int[] pointsAround = getPointsAround(indicesFlat.get(i));
      s.setEntry(2 * nb_edges + indicesRight.size() + i, 0,
    ...
    for (int i = 0; i < indicesHrAig.size(); ++i) {
      ...
      s.setEntry(2 * nb_edges + indicesRight.size() + indicesFlat.size() + i, 0,
    ...
    for (int i = 0; i < indicesHrObt.size(); ++i) {
      ...
      s.setEntry(2 * nb_edges + indicesRight.size() + indicesFlat.size() + indicesHrAig.size() + i, 0,
  ...
  private RealMatrix getP() {
    ...
    for (int i = 0; i < 2 * nb_edges; ++i)
      p.setEntry(i, i, poidsPtfFixe);
    for (int i = 2 * nb_edges; i < 2 * nb_edges + indicesRight.size(); ++i)
      p.setEntry(i, i, poids90);
    for (int i = 2 * nb_edges + indicesRight.size(); i < 2 * nb_edges + indicesRight.size() + indicesFlat.size(); ++i)
      p.setEntry(i, i, poids0);
    for (int i = 2 * nb_edges + indicesRight.size() + indicesFlat.size(); i < n; ++i)
      p.setEntry(i, i, poids45);
    return p;
  }

  // compute Dx = (A'PA)^1.(A'PB)
  private RealMatrix computeDx() {
    RealMatrix a = getA();
    RealMatrix atp = a.transpose().multiply(getP());
    // System.out.println("a " + a.getRowDimension() + " x " +
    // a.getColumnDimension());
    // System.out.println("p " + p.getRowDimension() + " x " +
    // p.getColumnDimension());
    // RealMatrix atpa_1 = new
    // LUDecomposition(at.multiply(p).multiply(a)).getSolver().getInverse();
    // RealMatrix a_1 = new
    // QRDecomposition(p.multiply(a)).getSolver().getInverse();
    RealMatrix dx = new QRDecomposition(atp.multiply(a)).getSolver().solve(atp.multiply(getB()));
    // RealMatrix dx =
    // MatrixUtils.inverse((at.multiply(p).multiply(a))).multiply(at.multiply(p).multiply(getB()));
    // Matrix dx =
    // (at.times(this.p).times(a)).inverse().times(at.times(p).times(getB()));
    // return dx;
    return dx;
  }

  // update points with x
  private void setNewPoints(RealMatrix x) {
    for (int i = 0; i < x.getRowDimension(); i = i + 2)
      points.set(i / 2, new DirectPosition(x.getEntry(i, 0), x.getEntry(i + 1, 0)));
    // updating Y too --potentially harmful
    // this.y = getY();
  }
  ...
  public IPolygon square() {
    RealMatrix x = this.xCurrent;
    // System.out.println("x " + x.getRowDimension() + "x" +
    // x.getColumnDimension());
    // System.out.println("b " + getB().getRowDimension() + "x" +
    ...
    } while (i < MAX_ITER && x.subtract(xCurrent).getFrobeniusNorm() > NORM_DIFF_TOL);
    nbIters = i;
  ...
  public static void main(String[] args) throws ParseException {
    ...
    pol = (IPolygon) WktGeOxygene.makeGeOxygene(
        "POLYGON((0.7470703125 48.94559401268371,0.9228515625 37.328235417231944,12.7001953125 36.62610836350546,23.5986328125 37.328235417231944,23.5107421875 44.615497679489586,22.5439453125 49.80395904562219,3.5870361328125 48.947037410050974,0.7470703125 48.94559401268371))");
    // pol = (IPolygon) WktGeOxygene.makeGeOxygene(
    // "POLYGON((342763.66688345832517371
    // 7685320.84001446887850761,342763.66688345832517371
    // 7685308.73941662814468145,342772.06036172935273498
    // 7685309.01919923722743988,342770.17182911833515391
    // 7685320.42034055478870869,342763.66688345832517371
    // 7685320.84001446887850761))");

    long begin = System.nanoTime();
    SquarePolygonLS sqls = new SquarePolygonLS(10, 0.11, 7);
    sqls.setPolygon(pol);
    IPolygon pol2 = sqls.square();
    long end = System.nanoTime();
    System.out.println();
    System.out.println("computed in " + (end - begin) / 1000000 + " ms");
    System.out.println("nb iters : " + sqls.getNbIters());
    System.out.println("LS: " + sqls.asWKT());
    System.out.println("original pol area " + pol.area());
    System.out.println("LS squared pol area " + pol2.area());
    System.out.println("-------------------------------------------");
    begin = System.nanoTime();
    SquarePolygonQR sqqr = new SquarePolygonQR(10, 0.11, 7);
    sqqr.setPolygon(pol);
    pol2 = sqqr.square();
    end = System.nanoTime();
    System.out.println();
    System.out.println("computed in " + (end - begin) / 1000000 + " ms");
    System.out.println("nb iters : " + sqqr.getNbIters());
    System.out.println("QR: " + sqqr.asWKT());
    System.out.println("original pol area " + pol.area());
    System.out.println("QR squared pol area " + pol2.area());
  }
