...
public class RoadNetworkMenuAgentComplement {
  ...
  private Logger logger = Logger
      .getLogger(RoadNetworkMenuAgentComplement.class.getName());

  private JLabel lblAgent = new JLabel("          AGENT");

  private JMenuItem mResRoutierCharger = new JMenuItem(new LoadRoadNetAction());
  private JMenuItem mResRoutierChargerTroncons = new JMenuItem(
      new LoadRoadSectionsAction());
  private JMenuItem mVoirResRoutierInfos = new JMenuItem(
      new DisplayInfoAction());
  private JMenuItem mResRoutierRetourEtatInitial = new JMenuItem(
      new RestoreAction());
  public JMenuItem mResRoutierDecomposer = new JMenuItem(new DecomposeAction());
  public JMenuItem mResRoutierNettoyerDecomposition = new JMenuItem(
      new CleanDecompositionAction());
  public JCheckBoxMenuItem mPointsResRoutierVoir = new JCheckBoxMenuItem(
      "Display points");
  public JCheckBoxMenuItem mSegmentsResRoutierVoir = new JCheckBoxMenuItem(
      "Display segments");
  ...
  private JMenuItem mRoutierMesoDecomposition = new JMenuItem(
      new MesoDecompositionAction());
  private JMenuItem mRoutierMesoRecomposition = new JMenuItem(
      new MesoRecompositionAction());
  private JMenuItem mRoutierDecomposition = new JMenuItem(
      new CoalDecompositionAction());
  ...
  private class LoadRoadSectionsAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      AgentGeneralisationScheduler.getInstance().initList();
      for (IRoadLine at : CartAGenDoc.getInstance().getCurrentDataset()
          .getRoads()) {
        RoadNetworkMenuAgentComplement.this.logger.info("Chargement de " + at);
        AgentGeneralisationScheduler.getInstance()
            .add(AgentUtil.getAgentFromGeneObj(at));
      }
    }

    public LoadRoadSectionsAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Load the sections of a road network agent");
  ...
  private class DisplayInfoAction extends AbstractAction {
    ...
    @Override
    public void actionPerformed(ActionEvent e) {
      ((RoadNetworkAgent) AgentUtil.getAgentFromGeneObj(
          CartAGenDoc.getInstance().getCurrentDataset().getRoadNetwork()))
              .printInfosConsole();
    }
    ...
    public DisplayInfoAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Display information on the road network agent");
  ...
  private class RestoreAction extends AbstractAction {
    ...
    public RestoreAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Restore the road network agent to the previous state");
  ...
  private class DecomposeAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      Thread th = new Thread(new Runnable() {
        ...
        public void run() {
          RoadNetworkMenuAgentComplement.this.logger
              .info("Decomposition de reseau routier");
          ((RoadNetworkAgent) AgentUtil.getAgentFromGeneObj(
              CartAGenDoc.getInstance().getCurrentDataset().getRoadNetwork()))
                  .decompose();
          RoadNetworkMenuAgentComplement.this.logger
              .info("Fin decomposition de reseau routier");
        }
      });
      th.start();
    }
    ...
    public DecomposeAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Decompose the road network agent into sub-micros");
  ...
  private class CleanDecompositionAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      Thread th = new Thread(new Runnable() {
        ...
        public void run() {
          RoadNetworkMenuAgentComplement.this.logger
              .info("Nettoyage decomposition de reseau routier");
          ((RoadNetworkAgent) AgentUtil.getAgentFromGeneObj(
              CartAGenDoc.getInstance().getCurrentDataset().getRoadNetwork()))
                  .cleanDecomposition();
          RoadNetworkMenuAgentComplement.this.logger
              .info("Fin nettoyage decomposition de reseau routier");
        }
      });
      th.start();
    }
    ...
    public void actionPerformed(ActionEvent e) {
      Thread th = new Thread(new Runnable() {
        ...
        public void run() {
          ...
          Set<IFeature> feats = SelectionUtil.getSelectedObjects(
          ...
          for (IFeature feat : feats) {
            ...
            IRoadSectionAgent roadAgent = (IRoadSectionAgent) AgentUtil
            ...
            ArrayList<ILineString> sectionsList = coalescenceSections
                .getSections();

            // decomposition de la route meso
            mesoController.decomposeIntoParts(sectionsList);

            for (CoalescenceConflictType type : coalescenceSections
                .getCoalescenceTypes()) {
      ...
      Thread th = new Thread(new Runnable() {
        ...
        public void run() {
          ...
          for (IFeature feat : feats) {
            ...
            ((MesoSectionAgent) roadAgent.getMesoAgent())
                .recomposeMesoSection();
  ...
  private class CoalDecompositionAction extends AbstractAction {
      ...
      Thread th = new Thread(new Runnable() {
        ...
        public void run() {
          for (IFeature sel : SelectionUtil.getSelectedObjects(
            ...
            RoadNetworkMenuAgentComplement.this.logger.info(
                "Segmentation par empatement sur le troncon routier " + sel);
            if (RoadNetworkMenuAgentComplement.this.logger
                .isLoggable(Level.CONFIG)) {
              RoadNetworkMenuAgentComplement.this.logger.config(
            ...
            for (ILineString ls : algo.getSections()) {
              IRoadLine road = CartAGenPlugin.getInstance().getGeneObjImpl()
                  .getCreationFactory()
    ...
    public CoalDecompositionAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Trigger coalescence decomposition on selected roads for tests");
  ...
  private class MinBreakAction extends AbstractAction {
    ...
    public MinBreakAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Trigger Min Break algorithm on selected roads for tests");
  ...
  private class MaxBreakAction extends AbstractAction {
    ...
    public MaxBreakAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Trigger Max Break algorithm on selected roads for tests");
  ...
  private class GalbeAction extends AbstractAction {
    ...
    public GalbeAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Trigger GALBE process on selected roads for tests");
  ...
  private class PlasterAction extends AbstractAction {
    ...
    public PlasterAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Trigger Plaster algorithm on selected roads for tests");
  ...
  private class AccordionAction extends AbstractAction {
    ...
    public AccordionAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Trigger Accordion algorithm on selected roads for tests");
  ...
  private class BendRemovalAction extends AbstractAction {
      ...
      Thread th = new Thread(new Runnable() {
        ...
        public void run() {
          for (IFeature sel : SelectionUtil.getSelectedObjects(
            ...
            RoadNetworkMenuAgentComplement.this.logger
                .info("Application de suppression de virage au troncon routier "
                    + sel);
            if (RoadNetworkMenuAgentComplement.this.logger
                .isLoggable(Level.CONFIG)) {
              RoadNetworkMenuAgentComplement.this.logger.config(
    ...
    public BendRemovalAction() {
      this.putValue(Action.SHORT_DESCRIPTION,
          "Trigger Bend removal algorithm on selected roads for tests");
