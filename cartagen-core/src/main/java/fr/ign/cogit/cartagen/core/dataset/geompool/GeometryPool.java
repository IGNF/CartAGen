...
public class GeometryPool {
    ...
    public void addFeatureToGeometryPool(IGeometry geom, Color color, int widthPixels) {
        ColouredFeature colFeat = new ColouredFeature(geom, color, widthPixels);
        ((IPopulation<IFeature>) dataset.getPopulation(CartAGenDataSet.GEOM_POOL)).add(colFeat);
     ...
     */
    @SuppressWarnings("unchecked")
    public void addPointFeatureToGeometryPool(IGeometry geom, Color color, int widthPixels, String symbolName) {
        ColouredFeature colFeat = new ColouredFeature(geom, color, widthPixels);
     ...
     */
    @SuppressWarnings("unchecked")
    public void addPolygonToGeometryPool(IPolygon geom, Color fillColor, Color strokeColor, int widthPixels) {
        ColouredFeature colFeat = new ColouredFeature(geom, fillColor, strokeColor, widthPixels);
        ((IPopulation<IFeature>) dataset.getPopulation(CartAGenDataSet.GEOM_POOL)).add(colFeat);
        Layer poolLayer = sld.getLayer(CartAGenDataSet.GEOM_POOL);
        if (poolLayer == null)
            return;
     ...
     */
    @SuppressWarnings("unchecked")
    public void addGraphToGeometryPool(IGraph graph, Color edgeColor, Color nodeColor) {
     ...
     */
    @SuppressWarnings("unchecked")
    public void addTriangulationToGeometryPool(Triangulation triangulation, Color edgeColor, Color nodeColor) {
    ...
    /**
     * Add a given vector to the geometry pool with the given color, at the
     * given position.
     * 
     * @param feat
     * @param color
     */
    @SuppressWarnings("unchecked")
    public void addVectorToGeometryPool(Vector2D vector, IDirectPosition startPt, Color color, int widthPixels) {
    ...
    public void addGridValueToPool(Map<Integer, Map<Integer, Double>> grid, Color color, IEnvelope env) {
        ...
        for (Integer i : grid.keySet()) {
            for (Integer j : grid.get(i).keySet()) {
                ...
                coords.add(new DirectPosition(env.minX() + j * cellSize, env.minY() + i * cellSize));
                coords.add(new DirectPosition(env.minX() + (j + 1) * cellSize, env.minY() + i * cellSize));
                coords.add(new DirectPosition(env.minX() + (j + 1) * cellSize, env.minY() + (i + 1) * cellSize));
                coords.add(new DirectPosition(env.minX() + j * cellSize, env.minY() + (i + 1) * cellSize));
                coords.add(new DirectPosition(env.minX() + j * cellSize, env.minY() + i * cellSize));

                addPolygonToGeometryPool(new GM_Polygon(new GM_LineString(coords)), cellColor, color, 2);
            }
        }
    }
}
