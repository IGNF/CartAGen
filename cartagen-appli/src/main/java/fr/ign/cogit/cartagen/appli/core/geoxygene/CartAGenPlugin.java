 ...
 */
public class CartAGenPlugin implements GeOxygeneApplicationPlugin, ActionListener {
    /**
     * Logger.
     */
    ...
    private JCheckBoxMenuItem displayRightPanel = new JCheckBoxMenuItem("Display cartaGen right panel");
     ...
     */
    private GeneObjImplementation currentGeneObjImpl = GeneObjImplementation.getDefaultImplementation();

    // config
    /**
     */
    public ConfigMenuComponent menuConfig = new ConfigMenuComponent("CartAGen-Config");
    ...
    /**
     * Relates a {@link CartAGenDB} to a {@link ProjectFrame} of the
    ...
    public void addDatabaseToFrame(CartAGenDB db) throws JAXBException {
        ...
        /*
         * testoss -- change le currentdataset quand on passe d'un projectframe
         * à l'autre
         */
        final ProjectFrame frameref = frame;
        String pjfType = frame.toString().contains("Floating") ? "floating" : "tabbed";
        System.out.println("type de projectframe : " + pjfType + " -- " + frameref);
        switch (pjfType) {
        case "floating":
            ((FloatingProjectFrame) frame).getInternalFrame().addInternalFrameListener(new InternalFrameAdapter() {
                public void internalFrameActivated(InternalFrameEvent e) {
                    // System.out.println(frameref.getDataSet());
                    CartAGenDoc.getInstance().setCurrentDataset((CartAGenDataSet) frameref.getSld().getDataSet());
                }
            });
            break;
        // FIXME ne fonctionne pas en l'état actuel
        case "tabbed":
            ((TabbedProjectFrame) frame).getGui().addFocusListener(new FocusAdapter() {
        ...
        StyledLayerDescriptor defaultSld = StyledLayerDescriptor
                .unmarshall(IGeneObj.class.getClassLoader().getResourceAsStream("xml/CartagenStyles.xml")); //$NON-NLS-1$
        CartAGenDoc.getInstance().getCurrentDataset().setSld(defaultSld);
        float opacity = 0.8f;
        float strokeWidth = 1.0f;
        for (GeographicClass geoClass : db.getClasses()) {
            String populationName = db.getDataSet().getPopNameFromFeatType(geoClass.getFeatureTypeName());
     ...
     * @param db
     */
    public ProjectFrame addDatabaseToFrame(CartAGenDB db, StyledLayerDescriptor defaultSld) {
        // s'il y a une seule project frame et qu'elle est vide, on la supprime
        if (application.getMainFrame().getDesktopProjectFrames().length == 1) {
            ProjectFrame frameIni = application.getMainFrame().getDesktopProjectFrames()[0];
            if (frameIni.getLayers().size() == 0) {
    ...
    public void saveWindows() {
        for (String dbName : CartAGenDoc.getInstance().getDatabases().keySet()) {
            ...
            IEnvelope displayEnvelope = frame.getLayerViewPanel().getViewport().getEnvelopeInModelCoordinates();
            IDirectPosition geoCenter = displayEnvelope.center();
            DatabaseView dbView = new DatabaseView(CartAGenDoc.getInstance(), geoCenter, displayEnvelope,
                    frame.getSld(), displayedLayers);
