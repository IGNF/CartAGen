...
public class DatabaseView {
    ...
    public int hashCode() {
        ...
        result = prime * result
                + ((geoCenter == null) ? 0 : geoCenter.hashCode());
        result = prime * result
                + ((localSld == null) ? 0 : localSld.hashCode());
        return result;
    }
    ...
    public void saveToXml(Element root, Document xmlDoc,
        ...
        n = xmlDoc.createTextNode(String
                .valueOf(this.getDisplayEnvelope().getLowerCorner().getX()));
        xlcElem.appendChild(n);
        lCornerElem.appendChild(xlcElem);
        Element ylcElem = xmlDoc.createElement("y");
        n = xmlDoc.createTextNode(String
                .valueOf(this.getDisplayEnvelope().getLowerCorner().getY()));
    ...
    private void readFromXml(Element xmlElem) {
        ...
        // the geoCenter
        Element centerElem = (Element) xmlElem
                .getElementsByTagName("geo-center").item(0);
        if (centerElem != null) {
            Element xCenterElem = (Element) centerElem.getElementsByTagName("x")
                    .item(0);
            double xCenter = Double.valueOf(
                    xCenterElem.getChildNodes().item(0).getNodeValue());
            Element yCenterElem = (Element) centerElem.getElementsByTagName("y")
                    .item(0);
            double yCenter = Double.valueOf(
                    yCenterElem.getChildNodes().item(0).getNodeValue());
        ...
        if (envElem != null) {
            ...
            Element xlcElem = (Element) lCornerElem.getElementsByTagName("x")
                    .item(0);
            double xlCorner = Double
                    .valueOf(xlcElem.getChildNodes().item(0).getNodeValue());
            Element ylcElem = (Element) lCornerElem.getElementsByTagName("y")
                    .item(0);
            double ylCorner = Double
                    .valueOf(ylcElem.getChildNodes().item(0).getNodeValue());
            Element uCornerElem = (Element) envElem
                    .getElementsByTagName("upper-corner").item(0);
            Element xucElem = (Element) uCornerElem.getElementsByTagName("x")
                    .item(0);
            double xuCorner = Double
                    .valueOf(xucElem.getChildNodes().item(0).getNodeValue());
            Element yucElem = (Element) uCornerElem.getElementsByTagName("y")
                    .item(0);
            double yuCorner = Double
                    .valueOf(yucElem.getChildNodes().item(0).getNodeValue());
        ...
        for (int i = 0; i < xmlElem.getElementsByTagName("displayed-layer")
                .getLength(); i++) {
            Element layerElem = (Element) xmlElem
                    .getElementsByTagName("displayed-layer").item(i);
