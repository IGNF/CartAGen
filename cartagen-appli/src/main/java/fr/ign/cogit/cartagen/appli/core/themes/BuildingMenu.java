...
public class BuildingMenu extends JMenu {
    ...
    private JMenuItem mBatimentSelectionnerTous = new JMenuItem(new SelectAction());

    public JCheckBoxMenuItem mBuildingDeplacement = new JCheckBoxMenuItem("Displace buildings");

    public JCheckBoxMenuItem mIdBatiVoir = new JCheckBoxMenuItem("Display id");

    public JCheckBoxMenuItem mVoirTauxSuperposition = new JCheckBoxMenuItem("Display overlapping rate");
    ...
    public JCheckBoxMenuItem mVoirAltitude = new JCheckBoxMenuItem("Voir altitude");

    public JCheckBoxMenuItem mVoirOrientationGenerale = new JCheckBoxMenuItem("Voir orientation generale");
    public JCheckBoxMenuItem mVoirOrientationMurs = new JCheckBoxMenuItem("Voir orientation murs");
    public JCheckBoxMenuItem mVoirRosaceOrientationMurs = new JCheckBoxMenuItem("Voir rosace orientation murs");

    public JCheckBoxMenuItem mVoirRosaceEncombrement = new JCheckBoxMenuItem("Voir rosace encombrement");

    public JCheckBoxMenuItem mVoirElongation = new JCheckBoxMenuItem("Voir elongation");

    public JCheckBoxMenuItem mVoirConvexite = new JCheckBoxMenuItem("Voir convexite");

    public JCheckBoxMenuItem mVoirLgPlusPetitCote = new JCheckBoxMenuItem("Voir lg plus petit cote");

    private JMenuItem mParametrisedDilatation = new JMenuItem(new ParametrisedDilatationAction());
    ...
    private JMenuItem mEnveloppeRectAxe = new JMenuItem(new ToRectAxisHullAction());
    ...
    private class SelectAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            for (IBuilding obj : CartAGenDoc.getInstance().getCurrentDataset().getBuildings()) {
                SelectionUtil.addFeatureToSelection(CartAGenPlugin.getInstance().getApplication(), obj);
            }
        }
    ...
    private class ParametrisedDilatationAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            Thread th = new Thread(new Runnable() {
                @Override
                public void run() {
                    String s = JOptionPane.showInputDialog(appli.getMainFrame().getGui(), "Coefficient de dilatation",
                    ...
                    for (IFeature sel : SelectionUtil.getSelectedObjects(appli)) {
                        ...
                        if (BuildingMenu.this.logger.isLoggable(Level.CONFIG)) {
                            BuildingMenu.this.logger.config("Geometrie initiale: " + ab.getGeom());
                        }
                        ab.setGeom(CommonAlgorithms.homothetie(ab.getGeom(), coef));
                        if (BuildingMenu.this.logger.isLoggable(Level.CONFIG)) {
                            BuildingMenu.this.logger.config("Geometrie finale: " + ab.getGeom());
                        }
        ...
        public ParametrisedDilatationAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Trigger parametrised dilatation algorithm on selected buildings");
    ...
    private class EnlargeToRectangleAction extends AbstractAction {
            ...
            Thread th = new Thread(new Runnable() {
                ...
                public void run() {
                    for (IFeature sel : SelectionUtil
                        ...
                        IBuilding ab = (IBuilding) sel;
                        BuildingMenu.this.logger.info("Transformation en PPRE du batiment " + ab);
                        if (BuildingMenu.this.logger.isLoggable(Level.CONFIG)) {
                            BuildingMenu.this.logger.config("Geometrie initiale: " + ab.getGeom());
                        }
                        ab.setGeom(SmallestSurroundingRectangleComputation.getSSR(ab.getGeom()));
                        if (BuildingMenu.this.logger.isLoggable(Level.CONFIG)) {
                            BuildingMenu.this.logger.config("Geometrie finale: " + ab.getGeom());
                        }
                        BuildingMenu.this.logger.info(" fin");
        ...
        public EnlargeToRectangleAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Trigger Enlarge to rectangle algorithm on selected buildings");
    ...
    private class ToRectangleAction extends AbstractAction {
            ...
            Thread th = new Thread(new Runnable() {
                ...
                public void run() {
                    for (IFeature sel : SelectionUtil
                        ...
                        IBuilding ab = (IBuilding) sel;
                        BuildingMenu.this.logger.info("Transformation en PPRE avec aire constante du batiment " + ab);
                        if (BuildingMenu.this.logger.isLoggable(Level.CONFIG)) {
                            BuildingMenu.this.logger.config("Geometrie initiale: " + ab.getGeom());
                        }
                        if (BuildingMenu.this.logger.isLoggable(Level.CONFIG)) {
                            BuildingMenu.this.logger.config("Aire initiale: " + ab.getGeom().area());
                        }
                        ab.setGeom(SmallestSurroundingRectangleComputation.getSSRPreservedArea(ab.getGeom()));
                        if (BuildingMenu.this.logger.isLoggable(Level.CONFIG)) {
                            BuildingMenu.this.logger.config("Geometrie finale: " + ab.getGeom());
                        }
                        if (BuildingMenu.this.logger.isLoggable(Level.CONFIG)) {
                            BuildingMenu.this.logger.config("Aire finale: " + ab.getGeom().area());
                        }
                        BuildingMenu.this.logger.info(" fin");
                    }
                }
        ...
        public ToRectangleAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Trigger Change to rectangle algorithm on selected buildings");
    ...
    private class RotateAction extends AbstractAction {
            ...
            Thread th = new Thread(new Runnable() {
                ...
                public void run() {
                    String s = JOptionPane.showInputDialog(
                            ...
                            "Angle de rotation (en degres, sens trigo)", "Cartagen", JOptionPane.PLAIN_MESSAGE);
                    double angle = 0.0;
                    if (s != null && !s.isEmpty()) {
                    ...
                    for (IFeature sel : SelectionUtil
                        ...
                        ab.setGeom(CommonAlgorithms.rotation(ab.getGeom(), angle * Math.PI / 180.0));
                        if (BuildingMenu.this.logger.isLoggable(Level.CONFIG)) {
                            BuildingMenu.this.logger.config("Geometrie finale: " + ab.getGeom());
                        }
                        BuildingMenu.this.logger.info(" fin");
        ...
        public RotateAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Trigger Rotate algorithm on selected buildings");
    ...
    private class ToConvexHullAction extends AbstractAction {
            ...
            Thread th = new Thread(new Runnable() {
                ...
                public void run() {
                    for (IFeature sel : SelectionUtil
                        ...
                        IBuilding ab = (IBuilding) sel;
                        BuildingMenu.this.logger.info("Transformation en enveloppe convexe du batiment " + ab);
                        if (BuildingMenu.this.logger.isLoggable(Level.CONFIG)) {
                            BuildingMenu.this.logger.config("Geometrie initiale: " + ab.getGeom());
                        }
        ...
        public ToConvexHullAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Trigger Change to convex hull algorithm on selected buildings");
                ...
                public void run() {
                    for (IFeature sel : SelectionUtil
                        ...
                        BuildingMenu.this.logger
                                .info("Transformation en enveloppe rectangulaire parallèle aux axes du batiment " + ab);
                        if (BuildingMenu.this.logger.isLoggable(Level.CONFIG)) {
                            BuildingMenu.this.logger.config("Geometrie initiale: " + ab.getGeom());
                        }
    ...
    private class SimplifyAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            Thread th = new Thread(new Runnable() {
                ...
                public void run() {
                    String s = JOptionPane.showInputDialog(
                            CartAGenPlugin.getInstance().getApplication().getMainFrame().getGui(), "Seuil", "Cartagen",
                    ...
                    for (IFeature sel : SelectionUtil
                        ...
                        if (BuildingMenu.this.logger.isLoggable(Level.CONFIG)) {
                            BuildingMenu.this.logger
                                    .config("Geometrie initiale: " + ab.getGeom().coord().size() + " " + ab.getGeom());
                        }
                        ab.setGeom(SimplificationAlgorithm.simplification(ab.getGeom(), coef));
                        if (BuildingMenu.this.logger.isLoggable(Level.CONFIG)) {
                            BuildingMenu.this.logger
                                    .config("Geometrie finale: " + ab.getGeom().coord().size() + " " + ab.getGeom());
                        }
                        BuildingMenu.this.logger.info(" fin");
                    }
                }
        ...
        public SimplifyAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Trigger Simplify algorithm on selected buildings");
    ...
    private class SimpleSquaringAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            Thread th = new Thread(new Runnable() {
                ...
                public void run() {
                    String s = JOptionPane.showInputDialog(
                            CartAGenPlugin.getInstance().getApplication().getMainFrame().getGui(),
                            "Angles to square (radians)", "Cartagen", JOptionPane.PLAIN_MESSAGE);
                    ...
                    for (IFeature sel : SelectionUtil
                        ...
                        PolygonSquaring squaring = new PolygonSquaring(ab.getGeom(), angTol, correctTol);
        ...
        public SimpleSquaringAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Trigger Simple Squaring algorithm on selected buildings");
    ...
    private class LSSquaringAction extends AbstractAction {
            ...
            Thread th = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    String s3 = JOptionPane.showInputDialog(
                            CartAGenPlugin.getInstance().getApplication().getMainFrame().getGui(), "45° angles tol (°)",
                            "Cartagen", JOptionPane.PLAIN_MESSAGE);
                    double midTol = 8.0;
                    ...
                    for (IFeature sel : SelectionUtil
                        ...
                        SquarePolygonLS squaring = new SquarePolygonLS(rightTol, 0.1, midTol);
        ...
        public LSSquaringAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Trigger Least Squares Squaring algorithm on selected buildings");
    ...
    private class AmalgamationAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            List<IFeature> selected = SelectionUtil
                    .getListOfSelectedObjects(CartAGenPlugin.getInstance().getApplication());
            IFeature feat1 = selected.get(0);
            IFeature feat2 = selected.get(1);
            PolygonAggregation algo = new PolygonAggregation((IPolygon) feat1.getGeom(), (IPolygon) feat2.getGeom());
        ...
        public AmalgamationAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Amalgamation of selected two buildings from Regnauld 98");
