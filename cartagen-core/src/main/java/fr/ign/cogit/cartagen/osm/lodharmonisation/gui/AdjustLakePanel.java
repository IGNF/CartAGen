...
import fr.ign.cogit.cartagen.osm.lodharmonisation.process.AdjustLakeOutlineToPaths;
...
public class AdjustLakePanel extends HarmonisationPanel {
  ...
  private IFeatureCollection<IFeature> paths, lakes;
  private JSpinner spinBuffer, spinShore, spinCushion, spinTol, spinAngle;
  private JCheckBox chkBridges;

  public AdjustLakePanel() {
    super();
    this.paths = new FT_FeatureCollection<IFeature>();
    this.lakes = new FT_FeatureCollection<IFeature>();
  ...
  public Set<IGeneObj> triggerHarmonisation(boolean window,
    ...
    PathNotCrossingLake detectionProcess = new PathNotCrossingLake(lakes, paths,
        lodSlider.getValue());
    ...
    AdjustLakeOutlineToPaths process = new AdjustLakeOutlineToPaths(
        ...
        (Double) spinTol.getValue(),
        (Double) spinAngle.getValue() * Math.PI / 180);
  ...
  private void fillCollections(boolean window, Set<IGeneObj> windowObjs) {
    if (window) {
      ...
      lakes.addAll(
          CartAGenDoc.getInstance().getCurrentDataset().getWaterAreas());
