...
import fr.ign.cogit.cartagen.appli.utilities.CartAGenProgressBar;
...
public class DatasetGeoxGUIComponent extends JMenu {
    ...
    /**
     * Action that allows to open a CartAGen dataset previously stored in XML
     * into the current document. The new dataset is put as current dataset.
     * 
     * @author GTouya
     * 
     */
    ...
    /**
     * Action to add a given shapefile as a layer of the current database. A
     * frame helps to choose if you want the shapefile to be a road, or a
     * building, etc. If there is no current database and/or document, default
     * ones are created.
     * 
     * @author GTouya
     * 
    ...
    /**
     * Action that allows to export data from the CartAGen classes to
     * shapefiles.
     * 
     * @author GTouya
     * 
    ...
    /**
     * Action that allows to export data from the CartAGen classes to PostGIS
     * tables.
     * 
     * @author GTouya
     * 
     */
    class ExportDataToPostGisAction extends AbstractAction {
    ...
    /**
     * Action that allows to overwrite some of the CartAGen classes by the
     * initial shapefiles of the DB.
     * 
     * @author GTouya
     * 
     */
    ...
    /**
     * Set the current dataset (the one that is used in generalisation
     * algorithms) from one of the datasets related to the databases of the
     * document.
     * 
     * @author GTouya
     * 
    ...
    /**
     * Action that propose to add a layer that is filled in the dataset but not
     * in the layerViewPanel.
     * 
     * @author GTouya
     * 
     */
    ...
    class AddLayerFrame extends JFrame
        ...
        private Set<String> selectedPops = new HashSet<>();
        private List<String> popsToSelect = new ArrayList<>();
        private Map<String, Class<? extends IGeometry>> geometryTypes = new HashMap<>();
        private JButton btnAdd, btnCancel;
        private StyledLayerDescriptor sld;

        AddLayerFrame(GeOxygeneApplication appli) {
            super("Add a layer to the current view");
            getFilledButNotDisplayedPops();
            this.setPreferredSize(new Dimension(250, 120));

            // a panel for layers to select
            JPanel p1 = new JPanel();
            ...
            int i = 1;
            for (String pop : popsToSelect) {
                // create a checkbox
                JCheckBox check = new JCheckBox(pop);
                check.addChangeListener(this);
                // get the column to add the checkbox
                if (i % 2 == 0) {
                    pRight.add(check);
                } else {
                    pLeft.add(check);
        ...
        private void getFilledButNotDisplayedPops() {
            ...
            for (IPopulation<? extends IFeature> pop : dataset
                ...
                if (pop.size() == 0)
                    continue;
                // check if pop is displayed in a layer
                if (sld.getLayer(pop.getNom()) != null)
                    continue;
        ...
        public void stateChanged(ChangeEvent e) {
            // get the checkbox related to the event
            JCheckBox check = (JCheckBox) e.getSource();
            if (check.isSelected())
                this.selectedPops.add(check.getText());
            else
                this.selectedPops.remove(check.getText());
        }
    ...
    class EditPersistentClassesFrame extends JFrame
        ...
        private JList<String> persistList;
        ...
        public EditPersistentClassesFrame() {
            ...
            this.persistList = new JList<String>();
        ...
        private DefaultComboBoxModel<String> buildComboModelFromProject() {
        ...
        public void stateChanged(ChangeEvent e) {
            ...
            if (this.chkPersist.isSelected()) {
                // fill the persistent classes with the existing IGeneObj
                // classes
