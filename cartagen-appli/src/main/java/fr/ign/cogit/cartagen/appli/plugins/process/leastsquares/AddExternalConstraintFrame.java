...
public class AddExternalConstraintFrame extends JFrame implements ActionListener {
    ...
    public AddExternalConstraintFrame(LeastSquaresFrame parent) throws HeadlessException {
        super();
        this.parent = parent;
        ...
        ImageIcon icone = new ImageIcon(this.getClass().getResource("/images/browse.jpeg"));
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().equals("OK")) {
            DefaultTableModel model = (DefaultTableModel) parent.tableContrRel.getModel();
            model.addRow(new String[] { ((Class<?>) cbConstraint.getSelectedItem()).getName(), txtClass1.getText(),
                    txtClass2.getText(), txtThreshold.getText() });
            parent.tableContrRel.setModel(model);
            if (!parent.contraintesActivees.contains(((Class<?>) cbConstraint.getSelectedItem()).getName())) {
                parent.contraintesActivees.add(((Class<?>) cbConstraint.getSelectedItem()).getName());
                parent.poidsContraintes.put(((Class<?>) cbConstraint.getSelectedItem()).getName(), new Double(1.0));
            }
