...
public class DEMPixel extends GeneObjPointDefault implements IDEMPixel {
  ...
  public void setX(double x) {
    this.x = x;
    this.setGeom(new GM_Point(
        new DirectPosition(x, this.getGeom().getPosition().getY())));
  }
  ...
  public void setY(double y) {
    this.y = y;
    this.setGeom(new GM_Point(
        new DirectPosition(this.getGeom().getPosition().getX(), y)));
  }
  ...
  public boolean intersecte(IEnvelope env) {
    return env.maxX() > this.getX()
        - GeneralisationSpecifications.getDEMResolution()
        && env.minX() < this.getX()
            + GeneralisationSpecifications.getDEMResolution()
        && env.maxY() > this.getY()
            - GeneralisationSpecifications.getDEMResolution()
