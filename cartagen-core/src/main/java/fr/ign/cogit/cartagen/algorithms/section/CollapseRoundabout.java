    ...
    for (IRoadLine road : externalRoads) {
      ...
      if (distFirst < distLast) {
        ...
        ((ArcReseau) road.getGeoxObj())
            .setNoeudInitial((NoeudReseau) node.getGeoxObj());
        ((NoeudReseau) node.getGeoxObj()).getArcsSortants()
            .add(((ArcReseau) road.getGeoxObj()));
      } else {
        ...
        // update the GeOx objects topology
        ((ArcReseau) road.getGeoxObj())
            .setNoeudFinal((NoeudReseau) node.getGeoxObj());
        ((NoeudReseau) node.getGeoxObj()).getArcsEntrants()
            .add(((ArcReseau) road.getGeoxObj()));
      }
