...
public class TownMenuAgentComplement {
    ...
    private class LoadAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (ITown obj : CartAGenDoc.getInstance().getCurrentDataset().getTowns()) {
                AgentGeneralisationScheduler.getInstance().add(AgentUtil.getAgentFromGeneObj(obj));
            }
        }

        public LoadAction() {
            this.putValue(Action.SHORT_DESCRIPTION, "Load all town agents into the scheduler");
    ...
    private class DeleteStreetsAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            selection.addAll(SelectionUtil.getSelectedObjects(CartAGenPlugin.getInstance().getApplication()));
            for (IFeature sel : selection) {
                ...
                StreetSelectionRuas action = new StreetSelectionRuas(townAgent, constraint, 1.0,
                        townAgent.getStreetNetwork(), GeneralisationSpecifications.ROADS_DEADEND_MIN_LENGTH
                ...
                if (townAgent.getStreetNetwork().getRoundabouts().size() == 0) {
                    ...
                    townAgent.getStreetNetwork().setRoundabouts(dataset.getRoundabouts().select(townAgent.getGeom()));
                }
                if (townAgent.getStreetNetwork().getStrokes().size() == 0) {
                    CartAGenDataSet dataset = CartAGenDoc.getInstance().getCurrentDataset();
                    AbstractCreationFactory factory = dataset.getCartAGenDB().getGeneObjImpl().getCreationFactory();
                    ...
                    for (Stroke stroke : network.getStrokes()) {
                        strokes.add(factory.createRoadStroke(stroke.getGeomStroke(), (RoadStroke) stroke));
                    }
                    townAgent.getStreetNetwork().setStrokes(strokes);
                }

                townAgent.getStreetNetwork().computeExternalAttributes(StreetNetworkParameters.costLarge,
